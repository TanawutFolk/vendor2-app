...
// React Query Imports
import { PREFIX_QUERY_KEY } from '@/_workspace/react-query/hooks/useFlowType'
...
const FlowTypeSearch = ({ setIsEnableFetching }: Props) => {
  // States
  ...
  const handleAdd = () => {
    const dataItem = {
      ...
      USER_PROFILE_SETTING_PROGRAM_DATA: {
        searchFilters: {
          FLOW_TYPE_NAME: getValues('searchFilters.FLOW_TYPE_NAME'),
          FLOW_TYPE_ALPHABET: getValues('searchFilters.FLOW_TYPE_ALPHABET'),
            ...
            <>
              <Grid container spacing={6}>
                <Grid item xs={12} sm={4} lg={4}>
                  <Controller
                    name='searchFilters.FLOW_TYPE_NAME'
                    control={control}
                    render={({ field }) => (
                      <CustomTextField
                        {...field}
                ...
                <Grid item xs={12} sm={4} lg={4}>
                  <Controller
                    name='searchFilters.FLOW_TYPE_ALPHABET'
                    control={control}
                    render={({ field }) => (
                      <CustomTextField
                        {...field}
