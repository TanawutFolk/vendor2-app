...
// React Query Imports
import { PREFIX_QUERY_KEY } from '@/_workspace/react-query/hooks/useUnitOfMeasurement'
...
const UnitOfMeasurementSearch = ({ setIsEnableFetching }: Props) => {
  // States
  ...
  const handleAdd = () => {
    const dataItem = {
      ...
      USER_PROFILE_SETTING_PROGRAM_DATA: {
        searchFilters: {
          UNIT_OF_MEASUREMENT_NAME: getValues('searchFilters.UNIT_OF_MEASUREMENT_NAME'),
          SYMBOL: getValues('searchFilters.SYMBOL'),
            ...
            <>
              <Grid container spacing={4}>
                <Grid item xs={12} sm={4} lg={4}>
                  <Controller
                    name='searchFilters.UNIT_OF_MEASUREMENT_NAME'
                    control={control}
                    render={({ field }) => (
                      <CustomTextField
                        {...field}
