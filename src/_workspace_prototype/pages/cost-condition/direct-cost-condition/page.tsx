...
// Components Imports
import DirectCostConditionSearch from './DirectCostConditionSearch'
import DirectCostConditionWatch from './DirectCostConditionWatch'
import DirectCostConditionTableData from './DirectCostConditionTableData'
        ...
        return {
          ...
          searchResults: {
            ...
            columnVisibility: {
              ...
              DIRECT_UNIT_PROCESS_COST:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnVisibility
                  .DIRECT_UNIT_PROCESS_COST ?? true,
              INDIRECT_RATE_OF_DIRECT_PROCESS_COST:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnVisibility
                  .INDIRECT_RATE_OF_DIRECT_PROCESS_COST ?? true,
              FISCAL_YEAR:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnVisibility
                  .FISCAL_YEAR ?? true,
              VERSION:
            ...
            columnFilterFns: {
              ...
              DIRECT_UNIT_PROCESS_COST:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnFilterFns
                  .DIRECT_UNIT_PROCESS_COST ?? MRT_FilterFns.contains.name,
              INDIRECT_RATE_OF_DIRECT_PROCESS_COST:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnFilterFns
                  .INDIRECT_RATE_OF_DIRECT_PROCESS_COST ?? MRT_FilterFns.contains.name,
              FISCAL_YEAR:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnFilterFns
    ...
    <Grid container spacing={6}>
      <FormProvider {...reactHookFormMethods}>
        ...
        >
          ...
          {isLoading ? null : <DirectCostConditionWatch />}
        </Grid>
        <Grid item xs={12}>
          <DirectCostConditionSearch setIsEnableFetching={setIsEnableFetching} />
        </Grid>
        <Grid item xs={12}>
          {isLoading ? (
            'Loading'
          ) : (
            <DirectCostConditionTableData
...
export default DirectCostCondition
