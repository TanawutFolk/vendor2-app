...
import IndirectCostConditionModal from './modal/IndirectCostConditionModal'
...
import { useSearch } from '@/_workspace/react-query/hooks/useIndirectCostCondition'
...
export interface ReturnApiSearchIndirectCostConditionI {
...
const getUrlParamSearch = ({
  ...
  FISCAL_YEAR,
  PRODUCT_MAIN
}: ParamApiSearchI): ReturnApiSearchIndirectCostConditionI => {
...
const IndirectCostConditionTableData = ({ isEnableFetching, setIsEnableFetching }: Props) => {
  const [openModalAdd, setOpenModalAdd] = useState<boolean>(false)
  const [openModalView, setOpenModalView] = useState<boolean>(false)

  const [rowSelected, setRowSelected] = useState<MRT_Row<IndirectCostConditionI> | null>(null)

  const handleClickOpen = () => setOpenModalAdd(true)
  const handleClickOpenModalView = (row: MRT_Row<IndirectCostConditionI>) => {
  ...
  const columns = useMemo<MRT_ColumnDef<IndirectCostConditionI>[]>(
    () => [
      {
      ...
      },
      {
        ...
        header: 'TOTAL INDIRECT COST (THB)',
        filterVariant: 'text',
        columnFilterModeOptions: ['endsWith', 'startsWith', 'contains', 'equals', 'notEquals'],
        filterFn: 'contains'
      },
...
export default IndirectCostConditionTableData
