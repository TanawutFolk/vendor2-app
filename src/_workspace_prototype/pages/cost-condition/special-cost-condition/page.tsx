...
// Components Imports
import SpecialCostWatch from './SpecialCostWatch'
import SpecialCostSearch from './SpecialCostSearch'
import SpecialCostTableData from './SpecialCostTableData'
...
const columns = ['PRODUCT_MAIN_NAME', 'ADJUST_PRICE', 'FISCAL_YEAR', 'VERSION', 'UPDATE_DATE', 'UPDATE_BY']

const SpecialCostCondition = () => {
        ...
        return {
          ...
          searchResults: {
            ...
            columnFilterFns: {
              ...
              ADJUST_PRICE:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnFilterFns
                  .ADJUST_PRICE ?? MRT_FilterFns.contains.name,
              FISCAL_YEAR:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnFilterFns
    ...
    <Grid container spacing={6}>
      <FormProvider {...reactHookFormMethods}>
        ...
        >
          ...
          {isLoading ? null : <SpecialCostWatch />}
        </Grid>
        <Grid item xs={12}>
          <SpecialCostSearch setIsEnableFetching={setIsEnableFetching} />
        </Grid>
        <Grid item xs={12}>
          {isLoading ? (
            'Loading'
          ) : (
            <SpecialCostTableData isEnableFetching={isEnableFetching} setIsEnableFetching={setIsEnableFetching} />
          )}
        </Grid>
      </FormProvider>
    </Grid>
...
export default SpecialCostCondition
