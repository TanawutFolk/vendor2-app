import { PREFIX_QUERY_KEY, useCreate } from '@/_workspace/react-query/hooks/useBomData'
import {
...
const BomAddModal = ({ openModalAdd, setOpenModalAdd, setIsEnableFetching }: AddModalProps) => {
  ...
  const [isLockedFlowProcess, setIsLockedFlowProcess] = useState(false)
  ...
  useEffect(() => {
    ...
    reactHookFormMethods.setValue('ITEM', {})
  }, [reactHookFormMethods.watch('PRODUCT_MAIN')])
  ...
  const onSubmit = () => {
    ...
    if (
      ...
      const message = {
        title: 'Add Bom',
        message: 'โปรดกรอก ข้อมูล Material in BOM ให้ครบถ้วน'
      }
  ...
  const handleAdd = () => {
    ...
    const dataItem = {
      ...
      TOTAL_COUNT_PROCESS: reactHookFormMethods.getValues('PROCESS')?.length ?? 0,
  ...
  const setProcess = (data: any, flowName: string) => {
    reactHookFormMethods.setValue('PROCESS', data)
    reactHookFormMethods.setValue('FLOW_NAME', flowName)
  ...
  const setBom = (data: any) => {
    ...
    reactHookFormMethods.setValue('FLOW_NAME', data.flowName)
                                  ...
                                  {reactHookFormMethods.watch('PROCESS').map((ps, index) => (
                                    <SortableItem id={index}>
                                      ...
                                      >
                                        ...
                                        >
                                          <Checkbox disabled={isLockedFlowProcess} checked={noSelected === ps.NO} />
                                        </TableCell>
