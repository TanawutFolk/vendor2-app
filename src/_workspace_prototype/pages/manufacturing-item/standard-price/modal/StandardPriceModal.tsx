...
const StandardPriceModal = (props: Props) => {
  ...
  const [errorMessageData, setErrorMessage] = useState([])
  ...
  const handleAdd = () => {
    ...
    data = data.map((item: addFormData) => {
      return {
        ...
        PURCHASE_PRICE_CURRENCY_SYMBOL: item.PURCHASE_PRICE_CURRENCY.CURRENCY_SYMBOL,
        PURCHASE_PRICE_UNIT_ID: item.ITEM_CODE.PURCHASE_UNIT_ID,
        VERSION_NO: item.ITEM_CODE.VERSION_NO,
        PURCHASE_UNIT_CODE: item.ITEM_CODE.PURCHASE_UNIT_CODE,
        USAGE_UNIT_CODE: item.ITEM_CODE.USAGE_UNIT_CODE,
        USAGE_UNIT_RATIO: item.ITEM_CODE.USAGE_UNIT_RATIO
      }
    })
  ...
  const onMutateSuccess = (data: any) => {
    if (data.data && data.data.Status == true) {
      ...
      setHasError(true)
      const formattedErrors = data?.data?.ResultOnDb.map((item: any) => ({
        ...item,
        errorMessage: item.errors ? Object.values(item.errors).join(', ') : data?.data?.Message || ''
      }))
