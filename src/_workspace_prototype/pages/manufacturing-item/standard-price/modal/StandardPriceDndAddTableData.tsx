...
import { PREFIX_QUERY_KEY, useCreateByImportFile } from '@/_workspace/react-query/hooks/useStandardPrice'
...
const StandardPriceDndAddTableData = ({
  ...
  const mutation = useCreateByImportFile(onMutateSuccess, onMutateError)
      ...
      try {
        ...
        const dataItem = {
          DATA: standardPriceJson
            .filter(item => {
              if (!item['Item Code'] || item['Item Internal Full Name'] === 'Please enter Item Code') {
            ...
            .map(item => {
              return {
                ...
                PURCHASE_PRICE_CURRENCY_SYMBOL: currencyJson.find(c => {
                  return c.CURRENCY_SYMBOL === item['Purchase Price Currency']
                })?.CURRENCY_SYMBOL,
                PURCHASE_PRICE_UNIT_ID: itemJson.find(i => {
                  return i.ITEM_CODE === item['Item Code']
                })?.PURCHASE_UNIT_ID,
                VERSION_NO: itemJson.find(i => {
                  return i.VERSION_NO === item['Version No.']
                })?.VERSION_NO,
                PURCHASE_UNIT_CODE: itemJson.find(i => {
                  return i.PURCHASE_UNIT_CODE === item['Purchase Unit Code']
                })?.PURCHASE_UNIT_CODE,
                USAGE_UNIT_CODE: itemJson.find(i => {
                  return i.USAGE_UNIT_CODE === item['Usage Unit Code']
                })?.USAGE_UNIT_CODE,
                USAGE_UNIT_RATIO: itemJson.find(i => {
                  return i.USAGE_UNIT_RATIO === item['Usage Unit Ratio']
                })?.USAGE_UNIT_RATIO
              }
            }),
        ...
        mutation.mutate(dataItem, {
          onSuccess: async (response: any) => {
            ...
            if (response?.headers?.status !== 'false') {
              ...
              const message = {
                title: 'Add Manufacturing Item Price',
                message: 'Import unsuccessfully. Please check the downloaded file for details.'
              }
    ...
    return [
      {
        ...
        Cell: ({ row }) => {
          ...
          return (
            <>
              <Typography color='error'>{errorMessageData[row.index]?.errorMessage}</Typography>{' '}
            </>
          )
        },
        size: 400
  ...
  }, [errors, JSON.stringify(watch('DATA')), errorMessageData])
