...
const searchFiltersSchema = z.object({
  ...
  flow: z
    .object({
      FLOW_ID: z
        .number({
          required_error: requiredFieldMessage({ fieldName: 'Flow' }),
          invalid_type_error: requiredFieldMessage({ fieldName: 'Flow' })
        })
        .int({ message: requiredFieldMessage({ fieldName: 'Flow' }) }) // value must be an integer
        .positive({ message: requiredFieldMessage({ fieldName: 'Flow' }) }), // > 0)
      FLOW_CODE: z.string({
        required_error: requiredFieldMessage({ fieldName: 'Flow' }),
        invalid_type_error: requiredFieldMessage({ fieldName: 'Flow' })
      }),
      FLOW_NAME: z
        ...
        .min(1, requiredFieldMessage({ fieldName: 'Flow' }))
    })
    .nullish()
    .nullable(),
  ...
  bom: z
    .object({
      BOM_ID: z
        .number({
          required_error: requiredFieldMessage({ fieldName: 'BOM' }),
          invalid_type_error: requiredFieldMessage({ fieldName: 'BOM' })
        })
        .int({ message: requiredFieldMessage({ fieldName: 'BOM' }) }) // value must be an integer
        .positive({ message: requiredFieldMessage({ fieldName: 'BOM' }) }), // > 0)
      BOM_CODE: z.string({
        required_error: requiredFieldMessage({ fieldName: 'BOM' }),
        invalid_type_error: requiredFieldMessage({ fieldName: 'BOM' })
      }),
      BOM_NAME: z
        ...
        .min(1, requiredFieldMessage({ fieldName: 'BOM' }))
    })
    .nullish()
    .nullable(),
  // ?? Product Group
  ...
  alreadyHaveSellingPrice: z
    .object({
      ...
      icon: z.string({
        required_error: requiredFieldMessage({ fieldName: 'Has Indirect Cost' }),
        invalid_type_error: requiredFieldMessage({ fieldName: 'Has Indirect Cost' })
      })
    })
    .nullish()
    .nullable(),
  hasIndirectCost: z
    .object({
      value: z
        ...
        .int({ message: requiredFieldMessage({ fieldName: 'Has Indirect Cost' }) }), // value must be an integer
      // .positive({ message: requiredFieldMessage({ fieldName: 'Already have a Item Price ?' }) }), // > 0)
      label: z
        .string({
          required_error: requiredFieldMessage({ fieldName: 'Has Indirect Cost' }),
        ...
        .min(1, requiredFieldMessage({ fieldName: 'Has Indirect Cost' })),
      icon: z.string({
        required_error: requiredFieldMessage({ fieldName: 'Has Indirect Cost' }),
        invalid_type_error: requiredFieldMessage({ fieldName: 'Has Indirect Cost' })
      })
    ...
    try {
      ...
      resolve({
        searchFilters: {
          ...
          hasIndirectCost:
            result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters?.hasIndirectCost,
          flow: result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters?.flow ?? null,
          bom: result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters?.bom ?? null
        },
        searchResults: {
