...
import ConfirmModalForSctForProduct from '../sct-for-product/modal/ConfirmModalForSctForProduct'
...
function SctBomExplosionTableData() {
  ...
  const paramForSearch: ParamApiSearchProductMainI = {
    SearchFilters: [
      ...
      {
        id: 'hasIndirectCost',
        value: getValues('searchFilters.hasIndirectCost')?.value ?? ''
      },
      {
  ...
  const columns = useMemo<MRT_ColumnDef<StandardCostI>[]>(
    () => [
      {
        accessorKey: 'ROOT_FILTERED_SCT',
        header: 'ROOT FILTERED SCT',
        Cell: ({ cell }) => {
