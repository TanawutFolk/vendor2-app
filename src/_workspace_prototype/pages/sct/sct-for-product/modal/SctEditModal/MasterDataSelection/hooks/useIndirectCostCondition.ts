...
const SCT_MASTER_DATA_SETTING_ID = 2 // indirect
const SERVICE = IndirectCostConditionServices
const TARGET_NAME = 'indirectCost.main.costCondition.indirectCostCondition'
const MENU_NAME = 'Indirect Cost Condition'
interface DataReturn {
  totalIndirectCost: number | undefined | null
...
export function useIndirectCostCondition(isLoading: boolean) {
  ...
  const [
    ...
    name: [
      ...
      'listSctMasterDataHistory',
      'sctTotalCost',
      'masterDataSelection.indirectCostCondition.SCT_RESOURCE_OPTION_ID',
      'indirectCost.main.costCondition.indirectCostCondition.version',
      'sctDetailForAdjust',
  ...
  const handleCalculationAlreadyCase = useCallback(async (): Promise<DataReturn | null> => {
    ...
    const { INDIRECT_COST_SALE_AVE } = sctTotalCost.find(item => item.IS_FROM_SCT_COPY == 0) ?? {}
    ...
    try {
      return {
        totalIndirectCost: INDIRECT_COST_SALE_AVE,
        fiscalYear,
        version
      }
    } catch (error) {
  ...
  const handleMasterDataCase = useCallback(
    async (signal: AbortSignal): Promise<DataReturn | null> => {
      ...
      try {
        ...
        const { TOTAL_INDIRECT_COST, VERSION } = res?.data?.ResultOnDb?.[0] ?? {}
        ...
        return {
          totalIndirectCost: adjust.totalIndirectCost.isAdjust
            ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.TOTAL_INDIRECT_COST
            : (TOTAL_INDIRECT_COST as number | undefined | null),
    ...
    [fiscalYear, productMainId, itemCategoryId, sctResourceOptionId, sctDetailForAdjust, adjust.totalIndirectCost]
  )
  ...
  const handleRevisionCase = useCallback(
    async (signal: AbortSignal): Promise<DataReturn | null> => {
      ...
      try {
        ...
        const { TOTAL_INDIRECT_COST } = res?.data?.ResultOnDb?.[0] ?? {}
    ...
    async (signal: AbortSignal): Promise<DataReturn | null> => {
      ...
      try {
        ...
        if (CREATE_FROM_isCalculationAlready) {
          const { INDIRECT_COST_SALE_AVE } = sctTotalCost?.find(item => item.IS_FROM_SCT_COPY == 1) ?? {}
