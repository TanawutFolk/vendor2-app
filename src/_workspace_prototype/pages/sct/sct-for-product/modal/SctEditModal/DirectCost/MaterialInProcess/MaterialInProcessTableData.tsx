...
const MaterialInProcessTableData = () => {
  ...
  const columns = useMemo<MRT_ColumnDef<BomFlowProcessItemUsageI>[]>(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) =>
          formatNumber(
            watch(`directCost.materialInProcess.main.body.${row.id}.price.yieldAccumulation`),
            2,
            false,
            '%'
          ),
      ...
      },
      {
        ...
        columnFilterModeOptions: ['endsWith', 'startsWith', 'contains', 'equals', 'notEquals'],
        filterFn: 'contains',
        Cell: ({ row }) => watch(`directCost.materialInProcess.main.body.${row.id}.price.purchasePriceCurrencyCode`)
      },
      {
  ...
  // console.log(getValues('directCost.materialInProcess.main.body'))
