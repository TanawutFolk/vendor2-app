...
export default function SctForProductEditModal({
  ...
  useEffect(() => {
    if (!sctData?.SCT_ID || !product?.productMain?.PRODUCT_MAIN_ID || !product?.itemCategory?.ITEM_CATEGORY_ID) return

    const controller = new AbortController() // สร้าง controller
      ...
      case 4: {
        ...
        inDirectCostCondition =
          await IndirectCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryIdAndRevisionNo({
            ...
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID,
            VERSION:
              listSctMasterDataHistory.find(item => item.SCT_MASTER_DATA_SETTING_ID == 2 && item.IS_FROM_SCT_COPY == 1)
                ?.VERSION_NO ?? 0
          })
            .then(res => {
              return res?.data?.ResultOnDb?.[0]
      ...
      case 2: {
        ...
        otherCostCondition =
          await OtherCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryIdAndRevisionNo({
            ...
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID,
            VERSION:
              listSctMasterDataHistory.find(item => item.SCT_MASTER_DATA_SETTING_ID == 3 && item.IS_FROM_SCT_COPY == 0)
                ?.VERSION_NO ?? 0
          }).then(res => {
            return res?.data?.ResultOnDb?.[0] ?? []
          })
      ...
      case 4: {
        ...
        otherCostCondition =
          await OtherCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryIdAndRevisionNo({
            ...
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID,
            VERSION:
              listSctMasterDataHistory.find(item => item.SCT_MASTER_DATA_SETTING_ID == 3 && item.IS_FROM_SCT_COPY == 1)
                ?.VERSION_NO ?? 0
          })
            .then(res => {
              return res?.data?.ResultOnDb?.[0] ?? []
      ...
      case 2: {
        ...
        specialCostCondition =
          await SpecialCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryIdAndRevisionNo({
            ...
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID,
            VERSION:
              listSctMasterDataHistory.find(item => item.SCT_MASTER_DATA_SETTING_ID == 4 && item.IS_FROM_SCT_COPY == 0)
                ?.VERSION_NO ?? 0
          }).then(res => {
            return res?.data?.ResultOnDb?.[0] ?? []
          })
      ...
      case 4: {
        ...
        specialCostCondition =
          await SpecialCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryIdAndRevisionNo({
            ...
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID,
            VERSION:
              listSctMasterDataHistory.find(item => item.SCT_MASTER_DATA_SETTING_ID == 4 && item.IS_FROM_SCT_COPY == 1)
                ?.VERSION_NO ?? 0
          })
            .then(res => {
              return res?.data?.ResultOnDb?.[0] ?? []
    ...
    return () => {
      controller.abort() // ยกเลิก request เมื่อ component unmount
    }
    // ??? 7 , 8 Manufacturing Item Price , Yield Rate Material
  }, [sctData?.SCT_ID, product.productMain.PRODUCT_MAIN_ID, product.itemCategory.ITEM_CATEGORY_ID])
