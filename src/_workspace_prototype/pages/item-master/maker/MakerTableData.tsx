...
import MakerAddModal from './modal/MakerAddModal'
import MakerEditModal from './modal/MakerEditModal'
import MakerDeleteModal from './modal/MakerDeleteModal'
import MakerViewModal from './modal/MakerViewModal'
...
import { useSearch } from '@/_workspace/react-query/hooks/useMakerData'
...
export interface ReturnApiSearchMakerI {
  MAKER_NAME: string
  INUSE: string | number
  Start: number
  Limit: number
...
//   MAKER_NAME = '',
//   inuseForSearch
// }: ParamApiSearchI): ReturnApiSearchMakerI => {
...
const MakerTableData = ({ isEnableFetching, setIsEnableFetching }: Props) => {
  const [openModalAdd, setOpenModalAdd] = useState<boolean>(false)
  const [openModalEdit, setOpenModalEdit] = useState<boolean>(false)
  const [openModalDelete, setOpenModalDelete] = useState<boolean>(false)
  const [openModalView, setOpenModalView] = useState<boolean>(false)

  const [rowSelected, setRowSelected] = useState<MRT_Row<MakerI> | null>(null)

  // Functions
  const handleClickOpen = () => setOpenModalAdd(true)
  const handleClickOpenModalView = (row: MRT_Row<MakerI>) => {
  ...
  //   MAKER_NAME: getValues('searchFilters.MAKER_NAME') || undefined,
  ...
  const paramForSearch: ParamApiSearchI = {
    SearchFilters: [
      {
        id: 'MAKER_NAME',
        value: getValues('searchFilters.MAKER_NAME').trim() || ''
      },
  ...
  const columns = useMemo<MRT_ColumnDef<MakerI>[]>(
    () => [
      {
