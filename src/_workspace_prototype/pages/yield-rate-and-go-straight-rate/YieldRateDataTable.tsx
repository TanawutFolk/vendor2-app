...
// Component Imports
import { fetchExportYieldRate, useSearch } from '@/_workspace/react-query/hooks/useYieldRateData'
...
const getUrlParamSearch = ({
  ...
  const params = {
    ...
    yieldRateOption: yieldRateOption,
...
function YieldRateDataTable({ isEnableFetching, setIsEnableFetching }: Props) {
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    return JSON.parse(localStorage.getItem('columnOrderYieldRate') || '[]')
  })
  ...
  const paramForSearch: ParamApiSearchYieldRateI = {
    ...
    PRODUCT_TYPE_NAME: getValues('searchFilters.PRODUCT_TYPE')?.PRODUCT_TYPE_NAME || '',
    PRODUCT_TYPE_CODE: getValues('searchFilters.PRODUCT_TYPE')?.PRODUCT_TYPE_CODE || '',
    FISCAL_YEAR: getValues('searchFilters.FISCAL_YEAR') ?? '',
    ITEM_CATEGORY_ID: getValues('searchFilters.ITEM_CATEGORY')?.ITEM_CATEGORY_ID || '',
    CUSTOMER_INVOICE_TO_ID: getValues('searchFilters.CUSTOMER_INVOICE_TO')?.CUSTOMER_INVOICE_TO_ID || '',
    SCT_REASON_SETTING_ID: getValues('searchFilters.SCT_REASON_SETTING')?.SCT_REASON_SETTING_ID || '',
    SCT_TAG_SETTING_ID: getValues('searchFilters.SCT_TAG_SETTING')?.SCT_TAG_SETTING_ID || '',
    IS_MODE: getValues('openViewMode') || false,
    ...
    yieldRateOption: getValues('searchFilters.yieldRateOption')?.value || ''
  ...
  const handleExportCurrentPage = async () => {
    ...
    try {
      const dataItem = {
        columnFilters: JSON.parse(localStorage.getItem('columnOrderYieldRate') || '[]'),
        columnVisibility: getValues('searchResults.columnVisibility'),
        DataForFetch: paramForSearch,
        TYPE: 'currentPage'
      }

      const file = await fetchExportYieldRate(dataItem)
      ...
      const filename = `YieldRate&GoStraightRate_${timestamp}.xlsx`
  ...
  const table = useMaterialReactTable({
    ...
    onColumnOrderChange: newOrder => {
      ...
      localStorage.setItem('columnOrderYieldRate', JSON.stringify(newOrder))
    },
    ...
    //   </div>
    // ),
    initialState: { showColumnFilters: getValues('searchResults.columnFilters')?.length ? true : false },
