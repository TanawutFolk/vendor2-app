...
const ImportPage = ({
  ...
  const handleImport = () => {
    ...
    // console.log('Group-By', Object.keys(groupedByProductType))
    ...
    for (let i = 0; i < Object.keys(groupedByProductType)?.length; i++) {
      ...
      listTotalYR = []
      listGSR = []
      const lastItem = element[element.length - 1]

      // console.log(lastItem)
      if (lastItem?.COLLECTION_POINT == null || lastItem?.COLLECTION_POINT == '') {
        const message = {
          title: 'Import Yield rate',
          message:
            'Please fill last Process Collection Point.\n' +
            'กรุณาระบุ Collection Point ของ Process สุดท้าย\n' +
            'PRODUCT TYPE CODE : ' +
            lastItem?.PRODUCT_TYPE_CODE,
          type: 'error'
        }
