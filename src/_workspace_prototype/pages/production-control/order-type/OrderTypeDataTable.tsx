...
import { useSearch } from '@/_workspace/react-query/hooks/useOrderType'
...
import OrderTypeAddModal from './modal/OrderTypeAddModal'
import OrderTypeViewModal from './modal/OrderTypeViewModal'
import OrderTypeEditModal from './modal/OrderTypeEditModal'
import OrderTypeDeleteModal from './modal/OrderTypeDeleteModal'
import SelectCustom from '@/components/react-select/SelectCustom'
import { useSettings } from '@/@core/hooks/useSettings'
import { MENU_ID } from './env'
import { useCheckPermission } from '@/_template/CheckPermission'

interface ParamApiSearchOrderTypeI extends ParamApiSearchResultTableI {
...
const getUrlParamSearch = ({
  ...
  inuseForSearch
}: ParamApiSearchOrderTypeI): object => {
  ...
  const params = {
    ...
    ORDER_TYPE_NAME: ORDER_TYPE_NAME.trim(),
    ORDER_TYPE_ALPHABET: ORDER_TYPE_ALPHABET.trim(),
...
function OrderTypeDataTable({ isEnableFetching, setIsEnableFetching }: Props) {
  // Hooks : react-hook-form
  const { getValues, control, setValue, watch } = useFormContext<FormData>()
  // States
  const [rowSelected, setRowSelected] = useState<MRT_Row<OrderTypeI> | null>(null)
  ...
  const handleClickOpenModalView = (row: MRT_Row<OrderTypeI>) => {
  ...
  // const paramForSearch: ParamApiSearchOrderTypeI = {
  ...
  //   UPDATE_BY: '',
  //   UPDATE_DATE: '',
  //   ORDER_TYPE_NAME: watch('searchFilters.ORDER_TYPE_NAME') || '',
  //   ORDER_TYPE_ALPHABET: watch('searchFilters.ORDER_TYPE_ALPHABET') || '',
  ...
  const paramForSearch: ParamApiSearchI = {
    SearchFilters: [
      {
        id: 'ORDER_TYPE_NAME',
        value: getValues('searchFilters.ORDER_TYPE_NAME') || ''
      },
      {
        id: 'ORDER_TYPE_ALPHABET',
        value: getValues('searchFilters.ORDER_TYPE_ALPHABET') || ''
      },
  ...
  // react-table
  const columns = useMemo<MRT_ColumnDef<OrderTypeI>[]>(
    () => [
      {
      ...
      {
        accessorKey: 'ORDER_TYPE_ALPHABET',
        header: 'ORDER TYPE ALPHABET',
        filterVariant: 'text',
        columnFilterModeOptions: ['endsWith', 'startsWith', 'contains', 'equals', 'notEquals'],
        filterFn: 'contains',
        enableColumnFilterModes: false
...
export default OrderTypeDataTable
