...
import ProcessEditModal from './modal/ProcessEditModal'
import ProcessDeleteModal from './modal/ProcessDeleteModal'
import ProcessViewModal from './modal/ProcessViewModal'
...
import { useSearch } from '@/_workspace/react-query/hooks/useProcessData'
...
export interface ReturnApiSearchProcessI {
...
const getUrlParamSearch = ({
  ...
  PROCESS_NAME = '',
  inuseForSearch
}: ParamApiSearchI): ReturnApiSearchProcessI => {
...
const ProcessTableData = ({ isEnableFetching, setIsEnableFetching }: Props) => {
  const [openModalAdd, setOpenModalAdd] = useState<boolean>(false)
  const [openModalEdit, setOpenModalEdit] = useState<boolean>(false)
  const [openModalDelete, setOpenModalDelete] = useState<boolean>(false)
  const [openModalView, setOpenModalView] = useState<boolean>(false)

  const [rowSelected, setRowSelected] = useState<MRT_Row<ProcessI> | null>(null)

  // Functions
  const handleClickOpen = () => setOpenModalAdd(true)
  const handleClickOpenModalView = (row: MRT_Row<ProcessI>) => {
  ...
  const paramForSearch: ParamApiSearchI = {
    ...
    PROCESS_CODE: getValues('searchFilters.PROCESS_CODE') || undefined,
    PROCESS_NAME: getValues('searchFilters.PROCESS_NAME') || undefined,
  ...
  const columns = useMemo<MRT_ColumnDef<ProcessI>[]>(
    () => [
      {
  ...
  const table = useMaterialReactTable({
    ...
    renderRowActions: ({ row }) => (
      <div className='flex items-center'>
        ...
        {row?.original?.INUSE_RAW_DATA === 0 || row?.original?.INUSE === 2 ? null : (
          <ActionsMenu
            row={row}
