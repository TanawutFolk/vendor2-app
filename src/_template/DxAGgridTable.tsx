...
import type { ColDef, ColGroupDef, GridState, IServerSideDatasource, StateUpdatedEvent } from 'ag-grid-community'
...
const disableColumnFilters = (columnDefs: Array<ColDef | ColGroupDef>): Array<ColDef | ColGroupDef> =>
  columnDefs.map(columnDef => {
    if ('children' in columnDef) {
      return {
        ...columnDef,
        children: disableColumnFilters(columnDef.children)
      }
    }
...
interface DxAGgridTableProps extends AgGridReactProps {
  ...
  /** Restore persisted column/sort state on mount */
...
export const DxAGgridTable = ({
  ...
  const mergedDefaultColDef = useMemo(() => {
    return {
      ...(defaultColDef ?? defaultDxColDef),
      filter: false,
      floatingFilter: false
    }
  }, [defaultColDef])

  const columnDefsWithoutFilters = useMemo(
    () => (rest.columnDefs ? disableColumnFilters(rest.columnDefs) : rest.columnDefs),
    [rest.columnDefs]
  )
  ...
  return (
    <Box sx={{ height, width: '100%', ...boxSx }}>
      <AgGridReact
        ...
        {...rest}
        columnDefs={columnDefsWithoutFilters}
