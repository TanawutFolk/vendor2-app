...
const PRESENTATION_BY_STEP_CODE: Record<string, RequestStatusPresentation> = {
  ...
  PO_PIC_IN_PROGRESS: { chipColor: 'warning', accent: '#FF9F43', icon: 'tabler-user-cog' },
...
const STEP_CODE_BY_STATUS_VALUE: Record<string, string> = {
  ...
  'pending agreement to vendor': 'PO_PIC_IN_PROGRESS',
  'po pic in progress': 'PO_PIC_IN_PROGRESS',
...
const STEP_CODE_ALIAS: Record<string, string> = {
  PENDING_AGREEMENT: 'PO_PIC_IN_PROGRESS'
}
...
export const getRequestStatusPresentation = (option?: Partial<RawStatusOption>): RequestStatusPresentation => {
  ...
  const stepCode = String(option.STEP_CODE ?? '')
    .trim()
    .toUpperCase()
  const resolvedStepCode =
    STEP_CODE_ALIAS[stepCode] || stepCode || STEP_CODE_BY_STATUS_VALUE[normalize(option.value || option.label)]
