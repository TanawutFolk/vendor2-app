...
import MasterDataSelectionModal from '../../components/CostConditionModal/SelectionModal'
import MasterDataOtherModal from '../../components/OtherModal/OtherModal'
import SctDataModal from '../../components/sctDataModal/SctDataModal'
...
const MasterDataSelection_Body = ({ name }: Props) => {
  ...
  // const LoaderNotSupportMultiple = () => {
    ...
    if (event.target.value === '1' || event.target.value === '3') {
      if (name === 'COST_CONDITION') {
        ...
        StandardCostForProductServices.getMaterialPriceData({
          RESOURCE_OPTION_ID: event.target.value,
          FISCAL_YEAR:
            sctPatternId === 1 ? Number(getValues('FISCAL_YEAR.value')) : Number(getValues('FISCAL_YEAR.value')) - 1,
...
export default MasterDataSelection_Body
