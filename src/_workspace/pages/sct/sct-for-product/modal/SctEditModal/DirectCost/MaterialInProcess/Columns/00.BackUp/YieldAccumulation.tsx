...
const YieldAccumulation = React.memo(
  (params: CustomCellRendererProps<FormDataPage['directCost']['materialInProcess']['main']['body'][0]>) => {
    ...
    const specificYieldRate = useMemo(() => {
      return (
        yieldRateData?.find(v => v.flowId === params?.data?.FLOW_ID && v.processId === params?.data?.PROCESS_ID)
          ?.yieldAccumulationForSct ?? ''
      )
    }, [yieldRateData, params?.data?.FLOW_ID, params?.data?.PROCESS_ID])
      ...
      try {
        ...
        if (isCalculationAlready) {
          const foundItem = listSctBomFlowProcessItemUsagePrice?.find(
            x =>
              x.BOM_FLOW_PROCESS_ITEM_USAGE_ID === params.data?.BOM_FLOW_PROCESS_ITEM_USAGE_ID &&
        ...
        if (params.data.ITEM_CATEGORY_ID_FROM_BOM === 5) {
          return 100
        }

        // 4. ITEM_CODE ขึ้นต้นด้วย 'C'
        if (
          params.data.ITEM_CODE_FOR_SUPPORT_MES?.startsWith('C') &&
          (params.data.ITEM_CATEGORY_ID_FROM_BOM === 4 || params.data.ITEM_CATEGORY_ID_FROM_BOM === 6)
        ) {
          return 100
        }

        // 5. Item - Material Yield Accumulation rate นั้น ถูก Adjust ด้วย Engineer
        if (typeof params.data.YIELD_ACCUMULATION_OF_ITEM_FOR_SCT === 'number') {
          return params.data.YIELD_ACCUMULATION_OF_ITEM_FOR_SCT
        }
