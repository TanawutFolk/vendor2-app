...
import OtherCostCondition from '@/_workspace/services/cost-condition/OtherCostConditionServices'

const SCT_MASTER_DATA_SETTING_ID = 3 // other cost
const SERVICE = OtherCostCondition
const TARGET_NAME = 'indirectCost.main.costCondition.otherCostCondition'
const MENU_NAME = 'Other Cost Condition'
interface DataReturn {
  ga: number | undefined | null
  margin: number | undefined | null
  sellingExpense: number | undefined | null
  vat: number | undefined | null
  cit: number | undefined | null
...
export function useOtherCostCondition(isLoading: boolean) {
  ...
  const [
    ...
    name: [
      ...
      'listSctMasterDataHistory',
      'sctTotalCost',
      'masterDataSelection.otherCostCondition.SCT_RESOURCE_OPTION_ID',
      'indirectCost.main.costCondition.otherCostCondition.version',
      'sctDetailForAdjust',
  ...
  const handleCalculationAlreadyCase = useCallback(async (): Promise<DataReturn | null> => {
    ...
    const { GA, MARGIN, SELLING_EXPENSE, CIT, VAT } = sctTotalCost.find(item => item.IS_FROM_SCT_COPY == 0) ?? {}
  ...
  const handleMasterDataCase = useCallback(
    async (signal: AbortSignal): Promise<DataReturn | null> => {
      ...
      try {
        ...
        const { GA, MARGIN, SELLING_EXPENSE, CIT, VAT, VERSION } = res?.data?.ResultOnDb?.[0] ?? {}
        ...
        return {
          ga: adjust.ga.isAdjust ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.GA : GA,
          margin: adjust.margin.isAdjust
            ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.MARGIN
            : MARGIN,
          sellingExpense: adjust.sellingExpense.isAdjust
            ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.SELLING_EXPENSE
            : SELLING_EXPENSE,
          cit: adjust.cit.isAdjust ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.CIT : CIT,
          vat: adjust.vat.isAdjust ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.VAT : VAT,
    ...
    [fiscalYear, productMainId, itemCategoryId, sctResourceOptionId, sctDetailForAdjust, adjust]
  )
  ...
  const handleRevisionCase = useCallback(
    async (signal: AbortSignal): Promise<DataReturn | null> => {
      ...
      try {
        ...
        const { GA, MARGIN, SELLING_EXPENSE, CIT, VAT } = res?.data?.ResultOnDb?.[0] ?? {}
    ...
    [fiscalYear, productMainId, itemCategoryId, version, sctResourceOptionId, sctDetailForAdjust, adjust]
  )
    ...
    async (signal: AbortSignal): Promise<DataReturn | null> => {
      ...
      try {
        ...
        if (CREATE_FROM_isCalculationAlready) {
          const { GA, MARGIN, SELLING_EXPENSE, CIT, VAT } = sctTotalCost?.find(item => item.IS_FROM_SCT_COPY == 1) ?? {}
          ...
          return {
            ga: adjust.ga.isAdjust
              ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.GA
              : (sctDetailForAdjust.find(item => item.IS_FROM_SCT_COPY == 1)?.GA ?? GA),
            margin: adjust.margin.isAdjust
              ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.MARGIN
              : (sctDetailForAdjust.find(item => item.IS_FROM_SCT_COPY == 1)?.MARGIN ?? MARGIN),
            sellingExpense: adjust.sellingExpense.isAdjust
              ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.SELLING_EXPENSE
              : (sctDetailForAdjust.find(item => item.IS_FROM_SCT_COPY == 1)?.SELLING_EXPENSE ?? SELLING_EXPENSE),
            cit: adjust.cit.isAdjust
              ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.CIT
              : (sctDetailForAdjust.find(item => item.IS_FROM_SCT_COPY == 1)?.CIT ?? CIT),
            vat: adjust.vat.isAdjust
              ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.VAT
              : (sctDetailForAdjust.find(item => item.IS_FROM_SCT_COPY == 1)?.VAT ?? VAT),
    ...
    try {
      ...
      if (result && !signal.aborted) {
        setValue(TARGET_NAME, {
          ...
          sellingExpense: result.sellingExpense,
          cit: result.cit,
          vat: result.vat,
