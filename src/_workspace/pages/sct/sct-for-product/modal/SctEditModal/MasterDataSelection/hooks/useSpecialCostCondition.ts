...
const SCT_MASTER_DATA_SETTING_ID = 4 // special cost
const SERVICE = SpecialCostConditionServices
const TARGET_NAME = 'indirectCost.main.costCondition.specialCostCondition'
const MENU_NAME = 'Special Cost Condition'
interface DataReturn {
  adjustPrice: number | undefined | null
...
export function useSpecialCostCondition(isLoading: boolean) {
  ...
  const [
    ...
    name: [
      ...
      'listSctMasterDataHistory',
      'sctTotalCost',
      'masterDataSelection.specialCostCondition.SCT_RESOURCE_OPTION_ID',
      'indirectCost.main.costCondition.specialCostCondition.version',
      'sctDetailForAdjust',
  ...
  const handleCalculationAlreadyCase = useCallback(async (): Promise<DataReturn | null> => {
    ...
    const { ADJUST_PRICE } = sctTotalCost.find(item => item.IS_FROM_SCT_COPY == 0) ?? {}
    ...
    try {
      return {
        adjustPrice: Number(formatNumber(ADJUST_PRICE, 4, false)), // ADJUST_PRICE,
        fiscalYear,
        version
      }
    } catch (error) {
  ...
  const handleMasterDataCase = useCallback(
    async (signal: AbortSignal): Promise<DataReturn | null> => {
      ...
      try {
        ...
        const { ADJUST_PRICE, VERSION } = res?.data?.ResultOnDb?.[0] ?? {}
        ...
        return {
          adjustPrice: adjust.adjustPrice.isAdjust
            ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.ADJUST_PRICE
            : (ADJUST_PRICE as number | undefined | null),
  ...
  const handleRevisionCase = useCallback(
    async (signal: AbortSignal): Promise<DataReturn | null> => {
      ...
      try {
        ...
        const { ADJUST_PRICE } = res?.data?.ResultOnDb?.[0] ?? {}
    ...
    async (signal: AbortSignal): Promise<DataReturn | null> => {
      ...
      try {
        ...
        if (CREATE_FROM_isCalculationAlready) {
          const { ADJUST_PRICE } = sctTotalCost?.find(item => item.IS_FROM_SCT_COPY == 1) ?? {}
          ...
          return {
            adjustPrice: adjust.adjustPrice.isAdjust
              ? sctDetailForAdjust.find(dataItem => dataItem.IS_FROM_SCT_COPY == 0)?.ADJUST_PRICE
              : (sctDetailForAdjust.find(item => item.IS_FROM_SCT_COPY == 1)?.ADJUST_PRICE ?? ADJUST_PRICE),
            fiscalYear,
            version:
              listSctMasterDataHistory?.find(
                item => item.SCT_MASTER_DATA_SETTING_ID == SCT_MASTER_DATA_SETTING_ID && item.IS_FROM_SCT_COPY == 1
    ...
    try {
      ...
      if (result && !signal.aborted) {
        setValue(TARGET_NAME, {
          adjustPrice: result.adjustPrice,
