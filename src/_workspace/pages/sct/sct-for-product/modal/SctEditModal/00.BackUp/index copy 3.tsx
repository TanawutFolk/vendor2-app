  ...
  const reactHookFormMethods = useForm<FormDataPage>({
    ...
    defaultValues: async (): Promise<FormDataPage> => {
      try {
        ...
        if (listSctComponentTypeResourceOptionSelect.filter(item => item.IS_FROM_SCT_COPY == 0).length !== 8) {
          setIsOpenModal(false)
          toast.error(
        ...
        const listSctBomFlowProcessItemUsagePrice = await SctBomFlowProcessItemUsagePriceServices.getBySctId({
        ...
        const listSctBomFlowProcessItemUsagePriceAdjust =
          await SctBomFlowProcessItemUsagePriceAdjustServices.getBySctId({
            SCT_ID: sctData.SCT_ID
          }).then(res => {
            return res.data.ResultOnDb
          })
        ...
        const importFeeRate = await ImportFeeServices.getByFiscalYear_MasterDataLatest({
