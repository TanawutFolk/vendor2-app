  ...
  const reactHookFormMethods = useForm<FormDataPage>({
    ...
    defaultValues: async (): Promise<FormDataPage> => {
      ...
      if (!rowSelected?.original?.SCT_ID) {
        ...
        // return empty defaults casted to FormDataPage
        return {} as FormDataPage
      }

      // helper for toast + early return
      const handleCriticalError = (msg: string) => {
        setIsOpenModal(false)
        toast.error(msg, { autoClose: 10000 })
      }

      // 1) fetch SCT data (needed for many subsequent calls)
      let sctData: any
      try {
        const res = await StandardCostForProductServices.getSctDataDetail({
          SCT_ID: rowSelected.original.SCT_ID
        })
        sctData = res?.data?.ResultOnDb?.[0]
      } catch (err: any) {
        handleCriticalError(`Not found data for SCT ID: ${rowSelected.original.SCT_ID}. Please check your data.`)
        return {} as FormDataPage
      }
      ...
      // 2) parallel fetches that depend only on sctData (or nothing)
      const parallelCalls = [
        // 0 sctCompare
        SctCompareServices.getBySctId({ SCT_ID: sctData.SCT_ID }),
        // 1 listFlowProcess
        FlowProcessServices.getByFlowId({ FLOW_ID: sctData.FLOW_ID }),
        // 2 listBomFlowProcessItemUsage
        BomFlowProcessItemUsageServices.getByBomId({ BOM_ID: sctData.BOM_ID }),
        // 3 sctDetailForAdjust
        SctDetailForAdjustService.getBySctId({ SCT_ID: sctData.SCT_ID }),
        // 4 sctTotalCost
        SctTotalCostService.getBySctId({ SCT_ID: sctData.SCT_ID }),
        // 5 listSctComponentTypeResourceOptionSelect
        SctComponentTypeResourceOptionSelectServices.getBySctId({ SCT_ID: sctData.SCT_ID }),
        // 6 listSctMasterDataHistory
        SctMasterDataHistoryServices.getBySctIdAndIsFromSctCopy({ SCT_ID: sctData.SCT_ID, IS_FROM_SCT_COPY: '' }),
        // 7 listSctBomFlowProcessItemUsagePrice
        SctBomFlowProcessItemUsagePriceServices.getBySctId({ SCT_ID: sctData.SCT_ID }),
        // 8 importFeeRate
        ImportFeeServices.getByFiscalYear_MasterDataLatest({ FISCAL_YEAR: sctData.FISCAL_YEAR })
      ]

      const settled = await Promise.allSettled(parallelCalls)

      // helper to extract data.ResultOnDb or first item
      const extractResult = (val: any) => {
        if (!val) return undefined
        return val?.data?.ResultOnDb ?? val
      }

      const [
        sctCompareRes,
        listFlowProcessRes,
        listBomFlowProcessItemUsageRes,
        sctDetailForAdjustRes,
        sctTotalCostRes,
        listSctComponentTypeResourceOptionSelectRes,
        listSctMasterDataHistoryRes,
        listSctBomFlowProcessItemUsagePriceRes,
        importFeeRateRes
      ] = settled.map(r => (r.status === 'fulfilled' ? extractResult(r.value) : undefined))
      ...
      const listFlowProcess = listFlowProcessRes ?? []
      const listBomFlowProcessItemUsage = listBomFlowProcessItemUsageRes ?? []
      const sctDetailForAdjust =
        (Array.isArray(sctDetailForAdjustRes) ? sctDetailForAdjustRes[0] : sctDetailForAdjustRes) ?? {}
      const sctTotalCost = (Array.isArray(sctTotalCostRes) ? sctTotalCostRes[0] : sctTotalCostRes) ?? {}
      const listSctComponentTypeResourceOptionSelect = listSctComponentTypeResourceOptionSelectRes ?? []
      const listSctMasterDataHistory = listSctMasterDataHistoryRes ?? []
      const listSctBomFlowProcessItemUsagePrice = listSctBomFlowProcessItemUsagePriceRes ?? []
      const importFeeRate: importFeeRateType =
        (Array.isArray(importFeeRateRes) ? importFeeRateRes[0] : importFeeRateRes) ?? {}

      // validate critical ones
      if (!listFlowProcess || listFlowProcess.length === 0) {
        handleCriticalError(`Flow Process not found for SCT ID: ${sctData.SCT_ID}. Please check your data.`)
        return {} as FormDataPage
      }

      if (!listBomFlowProcessItemUsage) {
        handleCriticalError(
          `BOM Flow Process Item Usage not found for SCT ID: ${sctData.SCT_ID}. Please check your data.`
        )
        return {} as FormDataPage
      }
      ...
      // 3) component-specific fetches (Direct, Indirect, Other, Special, Yield, ClearTime)
      // prepare helper to decide which API to call
      const getRevisionVersion = (settingId: number, isFromSctCopyFlag = 0) =>
        listSctMasterDataHistory.find(
          (it: any) => it.SCT_MASTER_DATA_SETTING_ID === settingId && it.IS_FROM_SCT_COPY === isFromSctCopyFlag
        )?.VERSION_NO ?? 0

      const componentPromises: Promise<any>[] = []

      const pushComponentFetch = (
        componentId: number,
        settingId: number,
        masterLatestFn: () => Promise<any>,
        revisionFn: (version: number) => Promise<any>
      ) => {
        const optionId = listSctComponentTypeResourceOptionSelect.find(
          (i: any) => i.SCT_COMPONENT_TYPE_ID == componentId
        )?.SCT_RESOURCE_OPTION_ID
        if (isCalculationAlready) {
          // if overall flag set, use revision (0) per original behavior
          componentPromises.push(revisionFn(getRevisionVersion(settingId, 0)))
          return
        }
        switch (optionId) {
          case 1:
            componentPromises.push(masterLatestFn())
            break
          case 2:
            componentPromises.push(revisionFn(getRevisionVersion(settingId, 0)))
            break
          case 4:
            componentPromises.push(revisionFn(getRevisionVersion(settingId, 1)))
            break
          default:
            // push resolved undefined to keep positions
            componentPromises.push(Promise.resolve(undefined))
        }
      }
      ...
      pushComponentFetch(
        ...
        1,
        () =>
          DirectCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryId_MasterDataLatest({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_MAIN_ID: sctData.PRODUCT_MAIN_ID,
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID
          }),
        (version: number) =>
          DirectCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryIdAndRevisionNo({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_MAIN_ID: sctData.PRODUCT_MAIN_ID,
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID,
            VERSION: version
      ...
      pushComponentFetch(
        ...
        2,
        () =>
          IndirectCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryId_MasterDataLatest({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_MAIN_ID: sctData.PRODUCT_MAIN_ID,
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID
          }),
        (version: number) =>
          IndirectCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryIdAndRevisionNo({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_MAIN_ID: sctData.PRODUCT_MAIN_ID,
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID,
            VERSION: version
      ...
      pushComponentFetch(
        ...
        3,
        () =>
          OtherCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryId_MasterDataLatest({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_MAIN_ID: sctData.PRODUCT_MAIN_ID,
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID
          }),
        (version: number) =>
          OtherCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryIdAndRevisionNo({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_MAIN_ID: sctData.PRODUCT_MAIN_ID,
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID,
            VERSION: version
      ...
      pushComponentFetch(
        ...
        4,
        () =>
          SpecialCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryId_MasterDataLatest({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_MAIN_ID: sctData.PRODUCT_MAIN_ID,
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID
          }),
        (version: number) =>
          SpecialCostConditionServices.getByProductMainIdAndFiscalYearAndItemCategoryIdAndRevisionNo({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_MAIN_ID: sctData.PRODUCT_MAIN_ID,
            ITEM_CATEGORY_ID: sctData.ITEM_CATEGORY_ID,
            VERSION: version
      ...
      pushComponentFetch(
        ...
        5,
        () =>
          YieldRateGoStraightRateTotalForSctServices.getByProductTypeIdAndFiscalYear_MasterDataLatest({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_TYPE_ID: sctData.PRODUCT_TYPE_ID
          }),
        (version: number) =>
          YieldRateGoStraightRateTotalForSctServices.getByProductTypeIdAndFiscalYearAndRevisionNo({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_TYPE_ID: sctData.PRODUCT_TYPE_ID,
            REVISION_NO: version
          })
      ...
      pushComponentFetch(
        ...
        6,
        () =>
          ClearTimeTotalForSctServices.getByProductTypeIdAndFiscalYear_MasterDataLatest({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_TYPE_ID: sctData.PRODUCT_TYPE_ID
          }),
        (version: number) =>
          ClearTimeTotalForSctServices.getByProductTypeIdAndFiscalYearAndRevisionNo({
            FISCAL_YEAR: sctData.FISCAL_YEAR,
            PRODUCT_TYPE_ID: sctData.PRODUCT_TYPE_ID,
            REVISION_NO: version
          })
      ...
      // run component-specific fetches in parallel
      const compSettled = await Promise.allSettled(componentPromises)
      const compResults = compSettled.map(r => (r.status === 'fulfilled' ? extractResult(r.value) : undefined))

      // unpack components in same sequence we pushed them:
      const [
        directCostConditionRes,
        inDirectCostConditionRes,
        otherCostConditionRes,
        specialCostConditionRes,
        yieldRateGoStraightRateTotalRes,
        clearTimeForSctTotalRes
      ] = compResults

      const directCostCondition: directCostConditionType = Array.isArray(directCostConditionRes)
        ? directCostConditionRes[0]
        : directCostConditionRes
      const inDirectCostCondition: inDirectCostConditionType = Array.isArray(inDirectCostConditionRes)
        ? inDirectCostConditionRes[0]
        : inDirectCostConditionRes
      const otherCostCondition: otherCostConditionType = Array.isArray(otherCostConditionRes)
        ? otherCostConditionRes[0]
        : otherCostConditionRes
      const specialCostCondition: specialCostConditionType = Array.isArray(specialCostConditionRes)
        ? specialCostConditionRes[0]
        : specialCostConditionRes
      const yieldRateGoStraightRateTotal: yieldRateGoStraightRateTotalType = Array.isArray(
        yieldRateGoStraightRateTotalRes
      )
        ? yieldRateGoStraightRateTotalRes[0]
        : yieldRateGoStraightRateTotalRes
      const clearTimeForSctTotal: clearTimeForSctTotalType = Array.isArray(clearTimeForSctTotalRes)
        ? clearTimeForSctTotalRes[0]
        : clearTimeForSctTotalRes

      // Note: ถ้าต้องการ behavior เดิมที่ toast + close modal เมื่อแต่ละ item ขาด ให้เพิ่ม checks แบบเดิมหลังจากนี้
      // assemble defaultValues object for the form
      // -> ปรับ key ให้ตรงกับ FormDataPage ของคุณ
      // const defaultValues = {
      //   sctData,
      //   sctCompare,
      //   sctCompareNo1: sctCompare.find((i: any) => i.SCT_COMPARE_NO == 1),
      //   sctCompareNo2: sctCompare.find((i: any) => i.SCT_COMPARE_NO == 2),
      //   listFlowProcess,
      ...
      //   clearTimeForSctTotal,
      //   listSctBomFlowProcessItemUsagePrice,
      //   importFeeRate
      // } as unknown as FormDataPage
      ...
      return {
        ...
        sctDetailForAdjust: {
          main: {
            sctTotalCost: {
              INDIRECT_COST_SALE_AVE: sctTotalCost.INDIRECT_COST_SALE_AVE,
              SELLING_EXPENSE: sctTotalCost.SELLING_EXPENSE,
              GA: sctTotalCost.GA,
              MARGIN: sctTotalCost.MARGIN,
              CIT: sctTotalCost.CIT,
              VAT: sctTotalCost.VAT,
              ADJUST_PRICE: sctTotalCost.ADJUST_PRICE,
              REMARK_FOR_ADJUST_PRICE: sctTotalCost.REMARK_FOR_ADJUST_PRICE,
              NOTE: sctTotalCost.NOTE,
              CIT_FOR_SELLING_PRICE: sctTotalCost.CIT_FOR_SELLING_PRICE,
              VAT_FOR_SELLING_PRICE: sctTotalCost.VAT_FOR_SELLING_PRICE,
              ESTIMATE_PERIOD_START_DATE: sctTotalCost.ESTIMATE_PERIOD_START_DATE,
              ESTIMATE_PERIOD_END_DATE: sctTotalCost.ESTIMATE_PERIOD_END_DATE,
              DIRECT_PROCESS_COST: sctTotalCost.DIRECT_PROCESS_COST,
              DIRECT_UNIT_PROCESS_COST: sctTotalCost.DIRECT_UNIT_PROCESS_COST,
              INDIRECT_RATE_OF_DIRECT_PROCESS_COST: sctTotalCost.INDIRECT_RATE_OF_DIRECT_PROCESS_COST
            },
            indirectCostCondition: {
              totalIndirectCost: {
                isAdjust: is_Null_Undefined_Blank(sctDetailForAdjust.TOTAL_INDIRECT_COST) ? false : true,
                isDisabledInput: true,
                dataFrom:
                  sctDetailForAdjust.TOTAL_INDIRECT_COST_SCT_RESOURCE_OPTION_ID == 1
                    ? 1
                    : sctDetailForAdjust.TOTAL_INDIRECT_COST_SCT_RESOURCE_OPTION_ID == 4
                      ? 4
                      : is_Null_Undefined_Blank(sctDetailForAdjust.TOTAL_INDIRECT_COST)
                        ? null
                        : 5
              }
            },
            otherCostCondition: {
              cit: {
                isAdjust: is_Null_Undefined_Blank(sctDetailForAdjust.CIT) ? false : true,
                isDisabledInput: true,
                dataFrom:
                  sctDetailForAdjust.CIT_SCT_RESOURCE_OPTION_ID == 1
                    ? 1
                    : sctDetailForAdjust.CIT_SCT_RESOURCE_OPTION_ID == 4
                      ? 4
                      : is_Null_Undefined_Blank(sctDetailForAdjust.CIT)
                        ? null
                        : 5
              },
              ga: {
                isAdjust: is_Null_Undefined_Blank(sctDetailForAdjust.GA) ? false : true,
                isDisabledInput: true,
                dataFrom:
                  sctDetailForAdjust.GA_SCT_RESOURCE_OPTION_ID == 1
                    ? 1
                    : sctDetailForAdjust.GA_SCT_RESOURCE_OPTION_ID == 4
                      ? 4
                      : is_Null_Undefined_Blank(sctDetailForAdjust.GA)
                        ? null
                        : 5
              },
              vat: {
                isAdjust: is_Null_Undefined_Blank(sctDetailForAdjust.VAT) ? false : true,
                isDisabledInput: true,
                dataFrom:
                  sctDetailForAdjust.VAT_SCT_RESOURCE_OPTION_ID == 1
                    ? 1
                    : sctDetailForAdjust.VAT_SCT_RESOURCE_OPTION_ID == 4
                      ? 4
                      : is_Null_Undefined_Blank(sctDetailForAdjust.VAT)
                        ? null
                        : 5
              },
              margin: {
                isAdjust: is_Null_Undefined_Blank(sctDetailForAdjust.MARGIN) ? false : true,
                isDisabledInput: true,
                dataFrom:
                  sctDetailForAdjust.MARGIN_SCT_RESOURCE_OPTION_ID == 1
                    ? 1
                    : sctDetailForAdjust.MARGIN_SCT_RESOURCE_OPTION_ID == 4
                      ? 4
                      : is_Null_Undefined_Blank(sctDetailForAdjust.MARGIN)
                        ? null
                        : 5
              },
              sellingExpense: {
                isAdjust: is_Null_Undefined_Blank(sctDetailForAdjust.SELLING_EXPENSE) ? false : true,
                isDisabledInput: true,
                dataFrom:
                  sctDetailForAdjust.SELLING_EXPENSE_SCT_RESOURCE_OPTION_ID == 1
                    ? 1
                    : sctDetailForAdjust.SELLING_EXPENSE_SCT_RESOURCE_OPTION_ID == 4
                      ? 4
                      : is_Null_Undefined_Blank(sctDetailForAdjust.SELLING_EXPENSE)
                        ? null
                        : 5
              }
            },
            specialCostCondition: {
              adjustPrice: {
                isAdjust: is_Null_Undefined_Blank(sctDetailForAdjust.ADJUST_PRICE) ? false : true,
                isDisabledInput: true,
                dataFrom:
                  sctDetailForAdjust.ADJUST_PRICE_SCT_RESOURCE_OPTION_ID == 1
                    ? 1
                    : sctDetailForAdjust.ADJUST_PRICE_SCT_RESOURCE_OPTION_ID == 4
                      ? 4
                      : is_Null_Undefined_Blank(sctDetailForAdjust.ADJUST_PRICE)
                        ? null
                        : 5
              }
            },
