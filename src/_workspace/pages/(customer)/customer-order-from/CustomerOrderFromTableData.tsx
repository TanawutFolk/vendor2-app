...
import { useSearchCustomerOrderFrom } from '@/_workspace/react-query/hooks/useCustomerOrderFromData'
...
import CustomerOrderFromAddModal from './modal/CustomerOrderFromAddModal'
import CustomerOrderFromEditModal from './modal/CustomerOrderFromEditModal'
import CustomerOrderFromDeleteModal from './modal/CustomOrderFromDeleteModal'

import CustomerOrderFromViewModal from './modal/CustomerOrderFromViewModal'
...
interface ParamApiSearchCustomerOrderFromI extends ParamApiSearchResultTableI_V2 {
  // PRODUCT_CATEGORY_ID?: number | ''
  CUSTOMER_ORDER_FROM_ID?: number | ''
  CUSTOMER_ORDER_FROM_NAME?: string
  CUSTOMER_ORDER_FROM_ALPHABET?: string
...
function CustomerOrderFromTableData({ isEnableFetching, setIsEnableFetching }: Props) {
  // react-hook-form
  const { getValues, control, setValue } = useFormContext<FormData>()

  // States
  const [rowSelected, setRowSelected] = useState<MRT_Row<CustomerOrderFromInterface> | null>(null)
  ...
  const [columnFilterFns, setColumnFilterFns] = useState<MRT_ColumnFilterFnsState>(
    getValues('searchResults.columnFilterFns') || {
      CREATE_BY: 'contains',
      CUSTOMER_ORDER_FROM_ID: 'contains',
      CUSTOMER_ORDER_FROM_NAME: 'contains',
      CUSTOMER_ORDER_FROM_ALPHABET: 'contains'
      //  PRODUCT_CATEGORY_ID: 'contains',
    }
  )
  ...
  // // react-query
  // const paramForSearch: ParamApiSearchCustomerOrderFromI = {
  ...
  //   UPDATE_BY: '',
  //   UPDATE_DATE: '',
  //   CUSTOMER_ORDER_FROM_NAME: getValues('searchFilters.customerOrderFromName') || '',
  //   CUSTOMER_ORDER_FROM_ALPHABET: getValues('searchFilters.customerOrderFromAlphabet'),
  ...
  const paramForSearch: ParamApiSearchCustomerOrderFromI = {
    SearchFilters: [
      {
        id: 'CUSTOMER_ORDER_FROM_NAME',
        value: getValues('searchFilters.customerOrderFromName').trim() || ''
      },
      {
        id: 'CUSTOMER_ORDER_FROM_ALPHABET',
        value: getValues('searchFilters.customerOrderFromAlphabet').trim() || ''
      },
  ...
  const { isRefetching, isLoading, data, isError, refetch, isFetching } = useSearchCustomerOrderFrom(
    paramForSearch,
    isEnableFetching
  )
  ...
  // react-table
  const columns = useMemo<MRT_ColumnDef<CustomerOrderFromInterface>[]>(
    () => [
      {
...

      {
        ...
        header: 'CUSTOMER ORDER FROM ALPHABET',
        filterVariant: 'autocomplete',
        columnFilterModeOptions: ['endsWith', 'startsWith', 'contains', 'equals', 'notEquals'],
        filterFn: 'contains',
        size: 400
  ...
  const table = useMaterialReactTable({
    ...
    // enableColumnFilters: false, //Closing MRT_ToggleFiltersButton
    ...
    renderTopToolbarCustomActions: () => (
      <div className='flex gap-1'>
        ...
        <Tooltip arrow title='Clear All Filters' onClick={() => table.resetColumnFilters(false)}>
          <IconButton>
...
export default CustomerOrderFromTableData
