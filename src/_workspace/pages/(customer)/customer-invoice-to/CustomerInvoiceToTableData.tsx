...
import { useSearchCustomerInvoiceTo } from '@/_workspace/react-query/hooks/useCustomerInvoiceToData'
...
import CustomerInvoiceToAddModal from './modal/CustomerInvoiceToAddModal'
import CustomerInvoiceToDeleteModal from './modal/CustomerInvoiceToDeleteModal'
import CustomerInvoiceToEditModal from './modal/CustomerInvoiceToEditModal'
import CustomerInvoiceToViewModal from './modal/CustomerInvoiceToViewModal'
import { MENU_ID } from './env'
import { useCheckPermission } from '@/_template/CheckPermission'

//or use your library of choice here

interface ParamApiSearchCustomerInvoiceToI extends ParamApiSearchResultTableI_V2 {
...
function CustomerInvoiceToTableData({ isEnableFetching, setIsEnableFetching }: Props) {
  // react-hook-form
  const { getValues, control, setValue } = useFormContext<FormData>()

  // States
  const [rowSelected, setRowSelected] = useState<MRT_Row<CustomerInvoiceToInterface> | null>(null)
  ...
  // // react-query
  // const paramForSearch: ParamApiSearchCustomerInvoiceToI = {
  ...
  //   UPDATE_BY: '',
  //   UPDATE_DATE: '',
  //   CUSTOMER_INVOICE_TO_NAME: getValues('searchFilters.customerInvoiceToName'),
  //   CUSTOMER_INVOICE_TO_ALPHABET: getValues('searchFilters.customerInvoiceToAlphabet'),
  ...
  const paramForSearch: ParamApiSearchCustomerInvoiceToI = {
    SearchFilters: [
      {
        id: 'CUSTOMER_INVOICE_TO_NAME',
        value: getValues('searchFilters.customerInvoiceToName') || ''
      },
      {
        id: 'CUSTOMER_INVOICE_TO_ALPHABET',
        value: getValues('searchFilters.customerInvoiceToAlphabet') || ''
      },
  ...
  const { isRefetching, isLoading, data, isError, refetch, isFetching } = useSearchCustomerInvoiceTo(
    paramForSearch,
    isEnableFetching
  ...
  // react-table
  const columns = useMemo<MRT_ColumnDef<CustomerInvoiceToInterface>[]>(
    () => [
      {
  ...
  const handleClickOpenModalView = (row: MRT_Row<CustomerInvoiceToInterface>) => {
...
export default CustomerInvoiceToTableData
