...
import { PREFIX_QUERY_KEY, useUpdateCustomerInvoiceTo } from '@/_workspace/react-query/hooks/useCustomerInvoiceToData'
...
interface CustomerInvoiceToEditModalProps {
  openModalEdit: boolean
  setOpenModalEdit: Dispatch<SetStateAction<boolean>>
  rowSelected: MRT_Row<CustomerInvoiceToInterface> | null
  setIsEnableFetching: Dispatch<SetStateAction<boolean>>
}

const CustomerInvoiceToEditModal = ({
  openModalEdit,
  setOpenModalEdit,
  rowSelected,
  setIsEnableFetching
}: CustomerInvoiceToEditModalProps) => {
  // useState
  ...
  const { control, handleSubmit, setValue, reset, getValues } = useForm<FormData>({
    ...
    defaultValues: {
      customerInvoiceToName: rowSelected?.original.CUSTOMER_INVOICE_TO_NAME,
      customerInvoiceToAlphabet: rowSelected?.original.CUSTOMER_INVOICE_TO_ALPHABET,
  ...
  const onMutateSuccess = data => {
    if (data.data && data.data.Status == true) {
      const message = {
        ...
        title: 'Update Customer Invoice To'
      }
  ...
  const mutation = useUpdateCustomerInvoiceTo(onMutateSuccess, onMutateError)
...
export default CustomerInvoiceToEditModal
