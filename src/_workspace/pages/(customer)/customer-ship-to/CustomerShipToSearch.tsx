...
import { PREFIX_QUERY_KEY } from '@/_workspace/react-query/hooks/useCustomerShipToData'
...
function CustomerShipToSearch({ setIsEnableFetching }: Props) {
  // States
  ...
  const handleAdd = () => {
    const dataItem = {
      ...
      USER_PROFILE_SETTING_PROGRAM_DATA: {
        searchFilters: {
          customerShipToName: getValues('searchFilters.customerShipToName'),
          customerShipToAlphabet: getValues('searchFilters.customerShipToAlphabet'),
            ...
            <>
              <Grid container spacing={4}>
                <Grid item xs={12} sm={4} lg={3}>
                  <Controller
                    name='searchFilters.customerShipToName'
                    control={control}
                    render={({ field }) => (
                      <CustomTextField
                        {...field}
                ...
                <Grid item xs={12} sm={4} lg={3}>
                  <Controller
                    name='searchFilters.customerShipToAlphabet'
                    control={control}
                    render={({ field }) => (
                      <CustomTextField
                        {...field}
...
export default CustomerShipToSearch
