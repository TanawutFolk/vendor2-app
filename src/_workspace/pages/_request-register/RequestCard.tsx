...
export const PRIMARY_BG =
  'linear-gradient(135deg, rgba(var(--mui-palette-primary-mainChannel) / 0.1) 0%, transparent 60%)'
  ...
  return (
    <Box
      ...
      sx={{
        ...
        background: isSelected ? cfg.bgGradient : 'var(--mui-palette-background-paper)',
        p: 2,
        ...
        <Box sx={{ display: 'flex', gap: 1.5, mb: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            <Typography variant='caption' color='text.disabled'>
              {request.contacts.length} contact{request.contacts.length !== 1 ? 's' : ''}
            </Typography>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <i className='tabler-package' style={{ fontSize: 11, color: 'var(--mui-palette-text-disabled)' }} />
            <Typography variant='caption' color='text.disabled'>
              {request.products.length} product{request.products.length !== 1 ? 's' : ''}
            </Typography>
          </Box>
        </Box>
