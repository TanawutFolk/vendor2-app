// Supplier / Outsourcing Selection Form (PDF template)
...
import type { SelectionFormData } from './useSelectionForm'
...
const s = StyleSheet.create({
  ...
  financialUndisclosed: {
    height: 148,
    width: '100%',
    alignItems: 'center',
    justifyContent: 'center'
  },
  financialUndisclosedText: {
    fontFamily: 'Helvetica-Bold',
    fontSize: 10,
    color: '#555'
  },
...
export function GprPdfDocument({
  form,
  rowData,
  chartDataUri,
  workflowStepIds,
  ...
  const hasFinancialData = (form.sales_profit || []).some(item =>
    [item?.total_revenue, item?.net_profit].some(value => String(value ?? '').trim() !== '')
  )

  const signatureSlots = buildSelectionSignatureSlots({
    approvalSteps: rowData?.APPROVAL_STEPS,
    approvalLogs: rowData?.APPROVAL_LOGS,
    workflowStepIds,
    approvalStepStatusIds,
    emptyDate: '........../........../..........'
  })
  ...
  return (
    <Document>
      <Page size='A4' style={s.page}>
        <Text fixed style={s.fixedRequestNumber}>
          {rowData?.REQUEST_NUMBER || rowData?.REQUEST_REGISTER_VENDOR_ID || ''}
        </Text>
        ...
        <View style={s.tbl}>
          {/* Address */}
          <View style={s.tblRow}>
            <Text style={[s.td, { width: 130, fontFamily: 'Helvetica-Bold', backgroundColor: '#F3E5FF' }]}>
              Address
            </Text>
          ...
          <View style={s.tblRow}>
            ...
            <Text style={[s.td, { width: 80, fontFamily: 'Helvetica-Bold', backgroundColor: '#F3E5FF' }]}>
              Start Year
            </Text>
          ...
          <View style={[s.tblRow, { alignItems: 'flex-start' }]}>
            <View style={s.financialChartCell}>
              ...
              {!hasFinancialData ? (
                <View style={s.financialUndisclosed}>
                  <Text style={s.financialUndisclosedText}>Supplier Undisclosed</Text>
                </View>
              ) : chartDataUri ? (
                <Image src={chartDataUri} style={s.financialChartImage} />
              ) : (
                <Text style={{ fontSize: 8, color: '#999', fontStyle: 'italic', margin: 20 }}>
                  Chart image will appear here
                </Text>
              )}
            </View>
          </View>
        ...
        <View style={s.tbl}>
          <View style={s.tblRow}>
            ...
            <Text style={[s.th, { width: 65 }]}>Judgment</Text>
            <Text style={[s.thl, { flex: 2 }]}>Remark</Text>
          </View>
          {form.criteria.map((c, i) => {
            ...
            return (
              <View key={c.no} style={s.tblRow}>
                ...
                <Text style={[Td, { width: 65 }]}>{c.files?.length ? 'OK' : '-'}</Text>
                <Text style={[Tdl, { flex: 2 }]}>
                  {(c.files || [])
        ...
        <View style={{ borderTopWidth: 1, borderTopColor: '#000', borderTopStyle: 'solid', paddingTop: 3 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'flex-end' }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'flex-end' }}>
              ...
              >
                {formatThaiDate(form.completion_date) || '  /  /  '}
              </Text>
            </View>
          </View>
        </View>
