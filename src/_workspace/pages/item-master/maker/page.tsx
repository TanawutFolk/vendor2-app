...
import MakerWatch from './MakerWatch'
import MakerTableData from './MakerTableData'
...
const columns = ['mrt-row-actions', 'INUSE', 'MAKER_ID', 'MAKER_NAME', 'MODIFIED_DATE', 'UPDATE_BY']
    ...
    defaultValues: async (): Promise<FormData> => {
      try {
        ...
        return {
          searchFilters: {
            MAKER_NAME:
              result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters.MAKER_NAME || '',
    ...
    <Grid container spacing={6}>
      <FormProvider {...reactHookFormMethods}>
        ...
        >
          ...
          {isLoading ? null : <MakerWatch />}
        </Grid>
        <Grid item xs={12}>
          <MakerSearch setIsEnableFetching={setIsEnableFetching} />
        </Grid>
        <Grid item xs={12}>
          {isLoading ? (
            'Loading'
          ) : (
            <MakerTableData isEnableFetching={isEnableFetching} setIsEnableFetching={setIsEnableFetching} />
          )}
        </Grid>
      </FormProvider>
    </Grid>
