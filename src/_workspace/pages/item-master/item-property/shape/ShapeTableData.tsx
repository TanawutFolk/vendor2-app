...
import ShapeAddModal from './modal/ShapeAddModal'
import ShapeEditModal from './modal/ShapeEditModal'
import ShapeDeleteModal from './modal/ShapeDeleteModal'
import ShapeViewModal from './modal/ShapeViewModal'
...
import { useSearch } from '@/_workspace/react-query/hooks/useShapeData'
...
export interface ReturnApiSearchShapeI {
  ITEM_PROPERTY_SHAPE_NAME: string
  INUSE: string | number
  Start: number
  Limit: number
...
const ShapeTableData = ({ isEnableFetching, setIsEnableFetching }: Props) => {
  const [openModalAdd, setOpenModalAdd] = useState<boolean>(false)
  const [openModalEdit, setOpenModalEdit] = useState<boolean>(false)
  const [openModalDelete, setOpenModalDelete] = useState<boolean>(false)
  const [openModalView, setOpenModalView] = useState<boolean>(false)

  const [rowSelected, setRowSelected] = useState<MRT_Row<ShapeI> | null>(null)

  // Functions
  const handleClickOpen = () => setOpenModalAdd(true)
  const handleClickOpenModalView = (row: MRT_Row<ShapeI>) => {
  ...
  const paramForSearch: ParamApiSearchI = {
    SearchFilters: [
      {
        id: 'ITEM_PROPERTY_SHAPE_NAME',
        value: getValues('searchFilters.ITEM_PROPERTY_SHAPE_NAME').trim() || ''
      },
  ...
  const columns = useMemo<MRT_ColumnDef<ShapeI>[]>(
    () => [
      {
      ...
      {
        accessorKey: 'ITEM_PROPERTY_SHAPE_ID',
        header: 'ITEM PROPERTY SHAPE ID',
