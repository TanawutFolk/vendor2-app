...
// React Query Imports
import { PREFIX_QUERY_KEY } from '@/_workspace/react-query/hooks/useShapeData'
...
const ShapeSearch = ({ setIsEnableFetching }: Props) => {
  // States
  ...
  const handleAdd = () => {
    const dataItem = {
      ...
      USER_PROFILE_SETTING_PROGRAM_DATA: {
        searchFilters: {
          ITEM_PROPERTY_SHAPE_NAME: getValues('searchFilters.ITEM_PROPERTY_SHAPE_NAME'),
            ...
            <>
              <Grid container spacing={4}>
                <Grid item xs={12} sm={4} lg={4}>
                  <Controller
                    name='searchFilters.ITEM_PROPERTY_SHAPE_NAME'
                    control={control}
                    render={({ field }) => (
                      <CustomTextField
                        {...field}
