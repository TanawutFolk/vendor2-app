...
import ColorAddModal from './modal/ColorAddModal'
import ColorEditModal from './modal/ColorEditModal'
import ColorDeleteModal from './modal/ColorDeleteModal'
import ColorViewModal from './modal/ColorViewModal'
...
import { useSearch } from '@/_workspace/react-query/hooks/useColorData'
...
export interface ReturnApiSearchColorI {
  ITEM_PROPERTY_COLOR_NAME: string
  INUSE: string | number
  Start: number
  Limit: number
...
//   inuseForSearch
// }: ParamApiSearchI): ReturnApiSearchColorI => {
...
//   let params = {
//     ITEM_PROPERTY_COLOR_NAME: ITEM_PROPERTY_COLOR_NAME || '',
...
const ColorTableData = ({ isEnableFetching, setIsEnableFetching }: Props) => {
  const [openModalAdd, setOpenModalAdd] = useState<boolean>(false)
  const [openModalEdit, setOpenModalEdit] = useState<boolean>(false)
  const [openModalDelete, setOpenModalDelete] = useState<boolean>(false)
  const [openModalView, setOpenModalView] = useState<boolean>(false)

  const [rowSelected, setRowSelected] = useState<MRT_Row<ColorI> | null>(null)

  // Functions
  const handleClickOpen = () => setOpenModalAdd(true)
  const handleClickOpenModalView = (row: MRT_Row<ColorI>) => {
  ...
  //   ITEM_PROPERTY_COLOR_NAME: getValues('searchFilters.ITEM_PROPERTY_COLOR_NAME') || undefined,
  ...
  const paramForSearch: ParamApiSearchI = {
    SearchFilters: [
      {
        id: 'ITEM_PROPERTY_COLOR_NAME',
        value: getValues('searchFilters.ITEM_PROPERTY_COLOR_NAME').trim() || ''
      },
  ...
  const columns = useMemo<MRT_ColumnDef<ColorI>[]>(
    () => [
      {
