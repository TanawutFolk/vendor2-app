...
const ImportDropzone = () => {
    ...
    onDrop: acceptedFiles => {
      ...
      reader.onload = function () {
        ...
        wb.SheetNames.forEach(function (sheetName) {
          ...
          let rangeSheet = 'A1:S1' + (range.e.r + 1).toString()
        ...
        for (let i = 0; i < watch('fileData')['content']?.length; i++) {
          ...
          if (element?.['YIELD_RATE'] == null || element?.['GO_STRAIGHT_RATE'] == null) {
      ...
      if (acceptedFiles.length && acceptedFiles[0].name.endsWith('xlsx')) {
        if (acceptedFiles[0].name.startsWith('YR-GSR')) {
  ...
  return (
    <Dropzone>
      <Card>
        {/* <CardHeader title='YR-GSR' sx={{ '& .MuiCardHeader-action': { alignSelf: 'center' } }} /> */}
        <CardContent>
