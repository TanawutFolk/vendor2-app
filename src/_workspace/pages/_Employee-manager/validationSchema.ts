  ...
  return new Promise(async resolve => {
    try {
      ...
      resolve({
        searchFilters: {
          // TODO: Can change this value
          keyword: result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters.keyword || '',
          groupCode: result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters.groupCode || null,
          inUse: result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters.inUse || ''
        },
        searchResults: {
          agGridState:
