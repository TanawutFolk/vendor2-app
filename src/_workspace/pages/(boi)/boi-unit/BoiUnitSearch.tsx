...
// Types
import {
  fetchBoiUnitNameByLikeBoiUnitNameAndInuse,
  fetchBoiUnitSymbolByLikeBoiUnitSymbolAndInuse
} from '@/_workspace/react-select/async-promise-load-options/boi/fetchBoiUnit'
...
function BoiUnitSearch({ setIsEnableFetching }: Props) {
  // States
    ...
    const dataItem = {
      ...
      USER_PROFILE_SETTING_PROGRAM_DATA: {
        searchFilters: {
          ...
          boiUnit: getValues('searchFilters.boiUnit'),
          boiUnitSymbol: getValues('searchFilters.boiUnitSymbol'),
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Grid container spacing={4}>
                ...
                <Controller
                  ...
                  render={({ field: { onChange, ...fieldProps } }) => (
                    <AsyncSelectCustom
                      ...
                      defaultOptions
                      loadOptions={inputValue => {
                        return fetchBoiUnitNameByLikeBoiUnitNameAndInuse(inputValue, 1)
                      }}
                      onChange={value => {
                        onChange(value)
                        // setValue('searchFilters.process', null)
                      }}
                      getOptionLabel={data => data?.BOI_UNIT_NAME || ''}
              ...
              <Grid item xs={12} sm={4} lg={3}>
                <Controller
                  name='searchFilters.boiUnitSymbol'
                  control={control}
                  render={({ field: { onChange, ...fieldProps } }) => (
                    <AsyncSelectCustom
                      {...fieldProps}
                      ...
                      defaultOptions
                      loadOptions={inputValue => {
                        return fetchBoiUnitSymbolByLikeBoiUnitSymbolAndInuse(inputValue, 1)
                      }}
                      onChange={value => {
                        onChange(value)
                        // setValue('searchFilters.process', null)
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, ...fieldProps } }) => (
                      <AsyncSelectCustom
                        ...
                        defaultOptions
                        loadOptions={inputValue => {
                          return fetchBoiProductGroupByLikeBoiProductGroupAndInuse(inputValue, 1)
                        }}
                        onChange={value => {
                          onChange(value)
                          // setValue('searchFilters.process', null)
