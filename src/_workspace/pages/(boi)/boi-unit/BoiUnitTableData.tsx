...
import ProductCategoryAddModal from './modal/BoiUnitAddModal'
import ProductCategoryEditModal from './modal/BoiUnitEditModal'
import ProductCategoryDeleteModal from './modal/BoiUnitDeleteModal'
import { useSearchBoiUnit } from '@/_workspace/react-query/hooks/useBoiUnitData'
import { BoiUnitI } from '@/_workspace/types/boi/BoiUnit'
import BoiUnitViewModal from './modal/BoiUnitViewModal'
...
interface ParamApiSearchBoiUnitI extends ParamApiSearchResultTableI_V2 {
  BOI_UNIT_ID?: number | ''
...
//   // INUSE = ''
//   // PRODUCT_CATEGORY_ID = ''
// }: ParamApiSearchBoiUnitI): any => {
//   const params = {
//     BOI_UNIT_NAME: BOI_UNIT_NAME.trim(),
//     BOI_UNIT_ID: BOI_UNIT_ID || '',
//     BOI_UNIT_SYMBOL: BOI_UNIT_SYMBOL || '',
...
function BoiUnitTableData({ isEnableFetching, setIsEnableFetching }: Props) {
  // react-hook-form
  const { getValues, control, setValue } = useFormContext<FormData>()

  // States
  const [rowSelected, setRowSelected] = useState<MRT_Row<BoiUnitI> | null>(null)
  ...
  // // react-query
  // const paramForSearch: ParamApiSearchBoiUnitI = {
  ...
  //   UPDATE_BY: '',
  //   UPDATE_DATE: '',
  //   BOI_UNIT_ID: getValues('searchFilters.boiUnit')?.BOI_UNIT_ID || '',
  //   BOI_UNIT_NAME: getValues('searchFilters.boiUnit')?.BOI_UNIT_NAME || '',
  //   BOI_UNIT_SYMBOL: getValues('searchFilters.boiUnitSymbol')?.BOI_UNIT_SYMBOL || '',
  ...
  const paramForSearch: ParamApiSearchBoiUnitI = {
    SearchFilters: [
      // {
      //   id: 'BOI_UNIT_ID',
      //   value: getValues('searchFilters.boiUnit')?.BOI_UNIT_ID || ''
      // },
      {
        id: 'BOI_UNIT_NAME',
        value: getValues('searchFilters.boiUnit')?.BOI_UNIT_NAME || ''
      },
      {
        id: 'BOI_UNIT_SYMBOL',
        value: getValues('searchFilters.boiUnitSymbol')?.BOI_UNIT_SYMBOL || ''
      },
  ...
  const { isRefetching, isLoading, data, isError, refetch, isFetching } = useSearchBoiUnit(
    paramForSearch,
    isEnableFetching
  ...
  // react-table
  const columns = useMemo<MRT_ColumnDef<BoiUnitI>[]>(
    () => [
      {
      ...
      {
        accessorKey: 'BOI_UNIT_NAME',
        header: 'BOI UNIT NAME',
  ...
  const handleClickOpenModalView = (row: MRT_Row<BoiUnitI>) => {
...
export default BoiUnitTableData
