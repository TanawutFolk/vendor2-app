...
import { useSearchBoiNameForMaterialConsumable } from '@/_workspace/react-query/hooks/useBoiNameForMaterialConsumableData'

import ProductCategoryAddModal from './modal/BoiNameForMaterialConsumableAddModal'
import ProductCategoryEditModal from './modal/BoiNameForMaterialConsumableEditModal'
import ProductCategoryDeleteModal from './modal/BoiNameForMaterialConsumableDeleteModal'

import { BoiNameForMaterialConsumableI } from '@/_workspace/types/boi/BoiNameForMaterialConsumable'
import BoiNameForMaterialConsumableViewModal from './modal/BoiNameForMaterialConsumableViewModal'
...
import { ParamApiSearchResultTableI_V2, SearchResultTableI } from '@/libs/material-react-table/types/SearchResultTable'
...
interface ParamApiSearchBoiNameForMaterialConsumableI extends ParamApiSearchResultTableI_V2 {
...
//   // INUSE = ''
//   // PRODUCT_CATEGORY_ID = ''
// }: ParamApiSearchBoiNameForMaterialConsumableI): any => {
//   const params = {
//     BOI_PROJECT_NAME: BOI_PROJECT_NAME.trim(),
//     BOI_PROJECT_ID: BOI_PROJECT_ID || '',
//     BOI_GROUP_NO: BOI_GROUP_NO || '',
...
function BoiNameForMaterialConsumableTableData({ isEnableFetching, setIsEnableFetching }: Props) {
  // react-hook-form
  const { getValues, control, setValue } = useFormContext<FormData>()

  // States
  const [rowSelected, setRowSelected] = useState<MRT_Row<BoiNameForMaterialConsumableI> | null>(null)
  ...
  // react-query
  // const paramForSearch: ParamApiSearchBoiNameForMaterialConsumableI = {
  ...
  //   BOI_GROUP_NO: getValues('searchFilters.boiGroupNo')?.BOI_GROUP_NO || '',
  ...
  const paramForSearch: ParamApiSearchBoiNameForMaterialConsumableI = {
    SearchFilters: [
      ...
      {
        id: 'BOI_GROUP_NO',
        value: getValues('searchFilters.boiGroupNo')?.BOI_GROUP_NO || ''
      },
  ...
  const { isRefetching, isLoading, data, isError, refetch, isFetching } = useSearchBoiNameForMaterialConsumable(
    paramForSearch,
    isEnableFetching
  ...
  // react-table
  const columns = useMemo<MRT_ColumnDef<BoiNameForMaterialConsumableI>[]>(
    () => [
      {
      ...
      {
        accessorKey: 'BOI_DESCRIPTION_MAIN_NAME',
        header: 'BOI Description Main Name',
        columnFilterModeOptions: ['endsWith', 'startsWith', 'contains', 'equals', 'notEquals'],

        filterFn: 'contains'
      },
      {
        accessorKey: 'BOI_DESCRIPTION_SUB_NAME',
        header: 'BOI Description Sub Name',
        columnFilterModeOptions: ['endsWith', 'startsWith', 'contains', 'equals', 'notEquals'],

        filterFn: 'contains'
      },
      {
        accessorKey: 'BOI_UNIT_SYMBOL',
        header: 'BOI UNIT SYMBOL',
        columnFilterModeOptions: ['endsWith', 'startsWith', 'contains', 'equals', 'notEquals'],
  ...
  const handleClickOpenModalView = (row: MRT_Row<BoiNameForMaterialConsumableI>) => {
  ...
  return (
    <>
      <Card>
        ...
        {openModalView ? (
          <BoiNameForMaterialConsumableViewModal
...
export default BoiNameForMaterialConsumableTableData
