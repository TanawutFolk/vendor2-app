...
const FlowProcessSelectModalTableData = ({
  ...
  const selectAction = async (row: any) => {
    ...
    if (get === 'flow') {
      ...
      const data = await fetchProcessByFlowProcessId(row.original.FLOW_ID).finally(() => {
        setIsLoadingFlow(false)
      })

      setProcessSelected(data, row.original.FLOW_NAME)
    }
    ...
    renderRowActions: ({ row }) => (
      <div className='flex items-center'>
        <Button
          ...
          disabled={row.original?.IS_DRAFT ? true : false || isLoadingFlow}
        >
          {isLoadingFlow ? 'Loading' : 'Select'}
        </Button>
      </div>
    ),
