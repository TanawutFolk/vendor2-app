...
import BomAddModal from './modal/BomAddModal'
import BomEditModal from './modal/BomEditModal'
import BomDeleteModal from './modal/BomDeleteModal'
import BomViewModal from './modal/BomViewModal'
import BomProductTypeModal from './modal/BomProductTypeModal'
...
const BomTableData = ({ isEnableFetching, setIsEnableFetching }: Props) => {
  const [openModalAdd, setOpenModalAdd] = useState<boolean>(false)
  const [openModalEdit, setOpenModalEdit] = useState<boolean>(false)
  const [openModalDelete, setOpenModalDelete] = useState<boolean>(false)
  const [openModalView, setOpenModalView] = useState<boolean>(false)
  const [openModalBomProductType, setOpenModalBomProductType] = useState<boolean>(false)

  const [rowSelected, setRowSelected] = useState<MRT_Row<BomI> | null>(null)

  // Functions
  const handleClickOpen = () => setOpenModalAdd(true)
  const handleClickOpenModalView = (row: MRT_Row<BomI>) => {
    setOpenModalView(true)
    setRowSelected(row)
  }
  const handleClickOpenModalBomProductType = (row: MRT_Row<BomI>) => {
    setOpenModalBomProductType(true)
    setRowSelected(row)
  }
  ...
  const paramForSearch: ParamApiSearchI = {
    ...
    PRODUCTION_PURPOSE_ID: getValues('searchFilters.PRODUCTION_PURPOSE')?.PRODUCTION_PURPOSE_ID,
    FLOW_ID: getValues('searchFilters.FLOW_PROCESS_CODE')?.FLOW_ID,
  ...
  const table = useMaterialReactTable({
    ...
    renderRowActions: ({ row }) => (
      <div className='flex items-center'>
        ...
        {row.original.INUSE === 0 && row.original.INUSE_RAW_DATA === 0 ? null : (
          <ActionsMenu
            row={row}
          ...
          >
            {row.original.IS_DRAFT === 1 ? null : (
              <>
                ...
                >
                  <ListItemIcon>
                    <i className='tabler-edit text-xl' color='primary' />
                  </ListItemIcon>
                  <ListItemText primary='Matching With Product Type' />
                </MenuItem>
              </>
            )}
          </ActionsMenu>
    ...
    <>
      <Card>
        ...
        {openModalBomProductType ? (
          <BomProductTypeModal
            openModalBomProductType={openModalBomProductType}
            setOpenModalBomProductType={setOpenModalBomProductType}
            rowSelected={rowSelected}
            setRowSelected={setRowSelected}
            setIsEnableFetching={setIsEnableFetching}
          />
