...
export const validationSchemaPage = z.object({
  ...
  searchResults: z.object({
    approvalGridState: z.any().optional(),
    actionRequiredGridState: z.any().optional()
  })
})
    ...
    try {
      ...
      resolve({
        searchFilters: {
          // TODO: Can change this value
          requestNumber:
            result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters.requestNumber || '',
          vendorName: result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters.vendorName || '',
          stepKeyword:
            result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters.stepKeyword || '',
        ...
        searchResults: {
          approvalGridState:
            result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.approvalGridState ||
            undefined,
          actionRequiredGridState:
            result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.actionRequiredGridState ||
            undefined
        }
      })
    } catch (error) {
      resolve({
        ...
        searchResults: {
          approvalGridState: undefined,
          actionRequiredGridState: undefined
        }
      })
    }
  })
