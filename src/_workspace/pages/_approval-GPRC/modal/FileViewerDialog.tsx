  ...
  return (
    ...
    >
      ...
      <DialogContent dividers sx={{ p: 0, display: 'flex', minHeight: 0 }}>
        {files.length === 0 ? (
          <Typography color='text.secondary' sx={{ p: 3, m: 'auto' }}>
            No files attached
          </Typography>
        ) : (
          <>
            {/* File list */}
            <Box
              sx={{ width: 280, flexShrink: 0, borderRight: '1px solid', borderColor: 'divider', overflowY: 'auto' }}
            >
              <List disablePadding>
                {files.map((file, idx) => (
                  <ListItemButton
