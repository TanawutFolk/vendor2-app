...
export default function RequestDetailDialog({
  ...
  useEffect(() => {
    if (detailQuery.isError) {
      ToastMessageError({
        ...
        message: detailQuery.error instanceof Error ? detailQuery.error.message : 'Failed to load request detail'
      })
    }
  ...
  const requestInfoItems = [
    {
      ...
      value: getValue(detail, fallback, 'supportProduct_Process', 'SUPPORTPRODUCT_PROCESS')
    },
  ...
  const vendorInfoItems = [
    ...
    { label: 'Trade Term', value: vendorRegion },
          ...
          {loading ? (
            ...
            <Box sx={{ p: 3 }}>
                  ...
                  <Grid container spacing={4}>
                    ...
                    {getValue(detail, fallback, 'address', 'ADDRESS') !== '-' && (
                      <Grid item xs={12}>
                        <ReadOnlyField
                          label='Address'
                          value={getValue(detail, fallback, 'address', 'ADDRESS')}
                          multiline
                        />
                      </Grid>
                    )}
                    ...
                    {products.map((product, index) => (
                      ...
                      >
                        <Grid item xs={12} sm={6}>
                          <ReadOnlyField
                            label='Product Group'
                            value={getValue(product, null, 'PRODUCT_GROUP', 'product_group')}
                          />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                        ...
                        <Grid item xs={12} sm={6}>
                          <ReadOnlyField
                            label='Product Name'
                            value={getValue(product, null, 'PRODUCT_NAME', 'product_name')}
                          />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                          <ReadOnlyField
                            label='Model List'
                            value={getValue(product, null, 'MODEL_LIST', 'model_list')}
                            multiline
                          />
                        </Grid>
                      </RecordCard>
                    ...
                    >
                      <Box sx={{ minWidth: 820 }}>
                        <Box
                          sx={{
                            display: 'grid',
                            gridTemplateColumns: '0.5fr 2.2fr 1.8fr 1.2fr 1.3fr',
                            px: 2,
                            py: 1,
                        ...
                        {gprCFlowSteps.map((step, index) => {
                          const statusCfg = getStepStatusCfg(step.M_APPROVAL_STEP_STATUS_ID, approvalStepStatusIds)
                          // GPR C steps are individual approvals, so surface the completed state as "Approved".
                          const stepStatusLabel =
                            Number(step.M_APPROVAL_STEP_STATUS_ID || 0) === approvalStepStatusIds.APPROVED
                            ? 'Approved'
                            : statusCfg.label
                          ...
                          return (
                            ...
                            >
                              <Typography variant='body2' fontWeight={600}>
                                {getValue(step, null, 'STEP_ORDER')}
                              </Typography>
                              <Typography variant='body2' fontWeight={600}>
                                {getValue(step, null, 'STEP_NAME', 'STEP_CODE')}
                              </Typography>
                              <Typography variant='body2' color='text.secondary'>
                                {getValue(step, null, 'APPROVER_NAME', 'APPROVER_EMPCODE')}
                              </Typography>
                              <Chip
              ...
              >
                ...
                >
                  <i
                    className='tabler-alert-triangle-filled'
                    style={{ fontSize: 20, color: 'var(--mui-palette-warning-main)', flexShrink: 0, marginTop: 2 }}
                  />
                  <Typography variant='body2' color='warning.dark' fontWeight={600}>
                    The vendor did not accept all GPR A conditions. Please review the GPR B file below, which reflects
                    the conditions the vendor has agreed to, before approving.
                  </Typography>
                </Box>
                {hasGprBFile ? (
                  <Box
                  ...
                  >
                    <i
                      className={getFileIcon(gprBDisplayName)}
                      style={{ fontSize: 24, color: 'var(--mui-palette-primary-main)' }}
                    />
                    <Typography
                      variant='body2'
                      fontWeight={700}
