...
const parseGprCList = (raw: unknown): unknown[] => {
  try {
...
export const hasCompletedGprCSetup = (row: Record<string, unknown>) => {
  ...
  return (
    hasText(row?.GPR_C_APPROVER_EMPCODE) &&
    hasText(row?.GPR_C_APPROVER_NAME) &&
    hasText(row?.GPR_C_APPROVER_EMAIL) &&
    hasText(row?.GPR_C_PC_PIC_EMPCODE) &&
    hasText(row?.GPR_C_PC_PIC_NAME) &&
    hasText(row?.GPR_C_PC_PIC_EMAIL) &&
    parseGprCList(row?.GPR_C_PRODUCT_CHECKERS || row?.GPR_C_PRODUCT_GROUP_CHECKERS).length > 0
  )
}
