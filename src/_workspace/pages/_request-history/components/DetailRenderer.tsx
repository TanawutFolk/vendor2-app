...
const DetailRenderer = ({ data }: { data: any }) => {
  ...
  const { requestStateIds } = useWorkflowIdentity()
  ...
  const workflowSteps = useMemo(() => {
    try {
      const approvalSteps =
        typeof data.APPROVAL_STEPS === 'string' ? JSON.parse(data.APPROVAL_STEPS) : data.APPROVAL_STEPS || []
    ...
    const isRejected = Number(data.M_REQUEST_STATE_ID || 0) === requestStateIds.REJECTED
    const currentStatusId = Number(data.CURRENT_M_REQUEST_STATUS_ID || 0)
    const currentStepIndex = isRejected
      ? -1
      : statusOptions.findIndex((status: any) => Number(status.M_REQUEST_STATUS_ID || 0) === currentStatusId)

    const s = statusOptions
      .map((s: any, idx: number) => {
        let stepState: any = 'pending'
        if (!isRejected && currentStepIndex >= 0) {
    ...
    return [submitted, ...s]
  }, [statusOptions, data.M_REQUEST_STATE_ID, data.CURRENT_M_REQUEST_STATUS_ID, data.APPROVAL_STEPS, requestStateIds.REJECTED])
  ...
  return (
    <Box sx={{ p: 4, bgcolor: 'background.default', borderBottom: '1px solid', borderColor: 'divider' }}>
      <Card
        variant='outlined'
        sx={{ border: '1px solid', borderColor: 'divider', boxShadow: '0 4px 18px rgba(0,0,0,0.06)' }}
      >
        <CardContent sx={{ p: '24px !important', display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              <Grid container spacing={4}>
                ...
                <Grid item xs={12}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: files.length > 0 ? 1.25 : 0 }}>
                    <i
                      className='tabler-paperclip'
                      style={{ fontSize: 15, color: 'var(--mui-palette-primary-main)' }}
                    />
          ...
          <Box>
            ...
            <StatusTimeline steps={workflowSteps} approvalSteps={approvalSteps} approvalLogs={approvalLogs} />
          </Box>
        </CardContent>
      </Card>
