...
const toRegisterStatus = (stepStatusId: unknown, ids: ApprovalStepStatusMasterIds): RegisterStatus => {
  const statusId = Number(stepStatusId || 0)
  if (statusId === ids.APPROVED) return 'completed'
  if (statusId === ids.IN_PROGRESS) return 'in_progress'
  if (statusId === ids.REJECTED) return 'rejected'
  if (statusId === ids.SKIPPED) return 'skipped'
  return 'pending'
}
      ...
      ? (() => {
          const mappedSteps = approvalSteps
            ...
            .map(s => {
              ...
              return {
                step: s.STEP_ORDER,
                workflowStepMasterId: s.WORKFLOW_STEP_MASTER_ID,
          ...
          const disagreedStepIds = new Set(
            [workflowStepIds.VENDOR_DISAGREED, workflowStepIds.ISSUE_GPR_B, workflowStepIds.ISSUE_GPR_C].filter(
          ...
          if (poPicInProgressStep && branchChildren.length > 0) {
            poPicInProgressStep.branchLabel = 'Disagreed Case'
            poPicInProgressStep.branchChildren = branchChildren
          }
              ...
              >
                ...
                {(step.description || step.updatedBy || step.updatedDate) && (
                  <Typography
                    ...
                    sx={{ display: 'block', mb: step.remark ? 1 : 0, lineHeight: 1.6 }}
                  >
                    {step.description}
                    {step.description && (step.updatedBy || step.updatedDate) ? ' • ' : ''}
