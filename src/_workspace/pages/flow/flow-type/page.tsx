...
// Components Imports
import FlowTypeSearch from './FlowTypeSearch'
import FlowTypeWatch from './FlowTypeWatch'
import FlowTypeTableData from './FlowTypeTableData'
...
const schema = object({
  searchFilters: object({
    FLOW_TYPE_NAME: nullable(string()),
    FLOW_TYPE_ALPHABET: nullable(string()),
    INUSE: nullable(
      object({
        value: number(),
        label: string()
    ...
    defaultValues: async (): Promise<FormData> => {
      try {
        ...
        return {
          searchFilters: {
            FLOW_TYPE_NAME:
              result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters.FLOW_TYPE_NAME || '',
            FLOW_TYPE_ALPHABET:
              result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchFilters.FLOW_TYPE_ALPHABET || '',
          ...
          searchResults: {
            ...
            columnFilterFns: {
              ...
              FLOW_TYPE_NAME:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnFilterFns
                  .FLOW_TYPE_NAME ?? MRT_FilterFns.contains.name,
              FLOW_TYPE_ALPHABET:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnFilterFns
                  .FLOW_TYPE_ALPHABET ?? MRT_FilterFns.contains.name,
              MODIFIED_DATE:
                result?.data?.ResultOnDb?.[0]?.USER_PROFILE_SETTING_PROGRAM_DATA?.searchResults.columnFilterFns
    ...
    <Grid container spacing={6}>
      <FormProvider {...reactHookFormMethods}>
        ...
        >
          ...
          {isLoading ? null : <FlowTypeWatch />}
        </Grid>
        <Grid item xs={12}>
          <FlowTypeSearch setIsEnableFetching={setIsEnableFetching} />
        </Grid>
        <Grid item xs={12}>
          {isLoading ? (
            'Loading'
          ) : (
            <FlowTypeTableData isEnableFetching={isEnableFetching} setIsEnableFetching={setIsEnableFetching} />
          )}
        </Grid>
      </FormProvider>
    </Grid>
