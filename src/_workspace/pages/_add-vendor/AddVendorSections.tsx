...
import { Controller, useFormContext, useFormState, useFieldArray, useWatch } from 'react-hook-form'
...
import type { DuplicateCheckResultType } from './modal/DuplicateCheckResultDialog'
...
import { defaultContactValues, defaultProductValues } from './validationSchema'
...
export const SectionCheck = ({ onVerifyChange, isVerified }: SectionCheckProps) => {
  ...
  const [resultModalType, setResultModalType] = useState<DuplicateCheckResultType>(null)

  const existingVendorDetailQuery = useFindVendorDetail(existingVendorId, editModalOpen)
  const { control, trigger, getValues, setValue } = useFormContext<FormDataPage>()
  const { errors } = useFormState({ control })
  const vendorRegion = useWatch({ control, name: 'vendor_region' }) || 'Local'
  const isOversea = vendorRegion === 'Oversea'
  ...
  const handleVerify = async () => {
    ...
    const isValid = await trigger(
      isOversea
        ? ['company_name', 'vendor_region', 'country']
        : ['company_name', 'vendor_region', 'province', 'postal_code']
    )
    if (isValid) {
      const values = getValues()
      checkDuplicate({
        company_name: values.company_name,
        vendor_region: values.vendor_region,
  ...
  return (
    <>
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} sm={isOversea ? 5 : 4}>
          <Controller
            name='company_name'
            control={control}
            render={({ field }) => (
          ...
          <Grid item xs={12}>
            ...
            >
              ...
              <Typography variant='body2' sx={{ mb: 2 }}>
                Company name matches {blacklistMatches.length} record(s) in the Blacklist. Contact your compliance team
                before proceeding.
              </Typography>
        ...
        <EditVendorModal
          ...
          rowData={existingVendorDetailQuery.data}
