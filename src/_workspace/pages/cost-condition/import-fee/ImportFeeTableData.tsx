...
import ImportFeeModal from './modal/ImportFeeModal'
...
import { useSearch } from '@/_workspace/react-query/hooks/useImportFee'
...
const getUrlParamSearch = ({
  ...
  const params = {
    ...
    ITEM_IMPORT_TYPE_ID: ITEM_IMPORT_TYPE ? ITEM_IMPORT_TYPE.ITEM_IMPORT_TYPE_ID : '',
...
const ImportFeeTableData = ({ isEnableFetching, setIsEnableFetching }: Props) => {
  const [openModalAdd, setOpenModalAdd] = useState<boolean>(false)
  const [openModalView, setOpenModalView] = useState<boolean>(false)

  const [rowSelected, setRowSelected] = useState<MRT_Row<ImportFeeI> | null>(null)

  const handleClickOpen = () => setOpenModalAdd(true)
  const handleClickOpenModalView = (row: MRT_Row<ImportFeeI>) => {
  ...
  const columns = useMemo<MRT_ColumnDef<ImportFeeI>[]>(
    () => [
      {
...
export default ImportFeeTableData
