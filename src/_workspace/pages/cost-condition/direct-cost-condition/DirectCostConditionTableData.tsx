...
import DirectCostConditionModal from './modal/DirectCostConditionModal'
...
import { useSearch } from '@/_workspace/react-query/hooks/useDirectCostCondition'
...
const DirectCostConditionTableData = ({ isEnableFetching, setIsEnableFetching }: Props) => {
  const [openModalAdd, setOpenModalAdd] = useState<boolean>(false)
  const [openModalView, setOpenModalView] = useState<boolean>(false)

  const [rowSelected, setRowSelected] = useState<MRT_Row<DirectCostConditionI> | null>(null)

  const handleClickOpen = () => setOpenModalAdd(true)
  const handleClickOpenModalView = (row: MRT_Row<DirectCostConditionI>) => {
  ...
  const columns = useMemo<MRT_ColumnDef<DirectCostConditionI>[]>(
    () => [
      {
      ...
      },
      {
        ...
        filterFn: 'contains',
        size: 400,
        Cell: ({ cell }) => <>{formatNumber(cell.getValue(), 2, true, '%')}</>
      },
      {
...
export default DirectCostConditionTableData
