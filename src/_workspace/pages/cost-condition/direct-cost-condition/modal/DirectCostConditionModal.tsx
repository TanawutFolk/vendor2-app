...
import { PREFIX_QUERY_KEY, useCreate } from '@/_workspace/react-query/hooks/useDirectCostCondition'
...
const schema = object({
  ...
  DIRECT_UNIT_PROCESS_COST: pipe(
    string(),
    nonEmpty('Direct Unit Process Cost is required'),
    regex(/^\d+(?:\.\d+)?$/, 'Invalid Number')
  ),
  INDIRECT_RATE_OF_DIRECT_PROCESS_COST: pipe(
    string(),
    nonEmpty('Indirect Rate of Direct Process Cost is required'),
...
type Props =
  ...
  | {
      ...
      rowSelected: MRT_Row<DirectCostConditionI> | null
      setRowSelected: Dispatch<SetStateAction<MRT_Row<DirectCostConditionI> | null>>
    }

const DirectCostConditionModal = (props: Props) => {
  const { mode } = props
    ...
    defaultValues:
      mode === 'add'
        ...
        : {
            ...
            DIRECT_UNIT_PROCESS_COST: props.rowSelected?.original.DIRECT_UNIT_PROCESS_COST,
            INDIRECT_RATE_OF_DIRECT_PROCESS_COST: props.rowSelected?.original.INDIRECT_RATE_OF_DIRECT_PROCESS_COST,
  ...
  const handleAdd = () => {
    ...
    const dataItem = {
      ...
      DIRECT_UNIT_PROCESS_COST: parseFloat(getValues('DIRECT_UNIT_PROCESS_COST')),
      INDIRECT_RATE_OF_DIRECT_PROCESS_COST: parseFloat(getValues('INDIRECT_RATE_OF_DIRECT_PROCESS_COST')),
  ...
  const onMutateSuccess = (data: any) => {
    if (data.data && data.data.Status == true) {
      const message = {
        ...
        title: 'Add Direct Cost Condition'
      }
      ...
      >
        <DialogTitle id='max-width-dialog-title'>
          <Typography variant='h5' component='span'>
            {mode === 'add' ? 'Add Direct Cost Condition' : 'View Direct Cost Condition'}
          </Typography>
        ...
        <DialogContent>
          <Grid spacing={4} container>
            ...
            <Grid item xs={12} sm={12} lg={12}>
              <Controller
                name='DIRECT_UNIT_PROCESS_COST'
                control={control}
                render={({ field: { onChange, ...fieldProps } }) => (
                  <CustomTextField
                    {...fieldProps}
                    sx={{ width: '100%' }}
                    label='Direct Unit Process Cost (THB)'
                    onChange={e => {
                      onChange(e.target.value)
                    }}
                    {...(errors.DIRECT_UNIT_PROCESS_COST && {
                      error: true,
                      helperText: errors.DIRECT_UNIT_PROCESS_COST.message
                    })}
                    disabled={mode === 'view'}
                    autoComplete='off'
                  />
            ...
            <Grid item xs={12} sm={12} lg={12}>
              <Controller
                name='INDIRECT_RATE_OF_DIRECT_PROCESS_COST'
                control={control}
                render={({ field: { onChange, ...fieldProps } }) => (
                  <CustomTextField
                    {...fieldProps}
                    ...
                    autoComplete='off'
                    {...(errors.INDIRECT_RATE_OF_DIRECT_PROCESS_COST && {
                      error: true,
                      helperText: errors.INDIRECT_RATE_OF_DIRECT_PROCESS_COST.message
                    })}
                    disabled={mode === 'view'}
                  />
                )}
...
export default DirectCostConditionModal
